...
type
  ...
  public
    ...
    property UserName: String read GetUserName write SetUserName;
    property Domain: string read GetDomain write SetDomain;
    property Mode: TRasDialerMode read FMode write FMode;
...
begin
  FPhoneBook:='hello';
  RasCheck(RasCreatePhonebookEntry(Wnd, PChar(FPhoneBook)));
  Refresh;
end;
...
begin
  FConnHandle := 0;
  if FMode=dmAsync then begin
    RasCheck(RasDial(nil, nil, @FParams, $FFFFFFFF, Pointer(FNotifyWnd), FConnHandle));
  end else begin
    RasCheck(RasDial(nil, nil, @FParams, 0, nil, FConnHandle));
  end;
end;
...
function TRasDialer.GetDomain: String;
begin
  Result := FParams.szDomain;
end;
...
procedure TRasDialer.SetDomain(const Value: String);
begin
  StrPCopy(FParams.szDomain, Value);
end;
