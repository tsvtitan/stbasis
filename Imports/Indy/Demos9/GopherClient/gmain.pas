...
uses
  ...
  ComCtrls, IdComponent, IdTCPConnection, IdTCPClient, IdGopher, StdCtrls,
  IdBaseComponent, ExtCtrls;
...
type
  ...
  private
    ...
    procedure GetItem ( ItemType : Char );
...
procedure TfrmGopher.GetItem(ItemType: Char);
begin
  case ItemType of
    IdGopherItem_Document : GetTextFile;
    IdGopherItem_Directory : GetMenu;
{
...
begin
  ...
  try
    ...
    edtSelector.Text := idDemoGopher.Selector;
  finally
    FreeAndNil ( mnu );
  end; //try..finally
end;
...
begin
  ...
  try
    {This gets the file }
    idDemoGopher.GetFile(FileStream);
    dia := TfrmTextView.Create ( Application );
    try
      dia.redtTextFile.Lines.LoadFromStream ( FileStream );
      dia.ShowModal;
    finally
      FreeAndNil ( dia );
    end;
...
begin
  ...
  begin
    ...
    {SubItems[3]=Selector}
    idDemoGopher.Selector := lvGopherMenu.Selected.SubItems[ 3 ];
    {SubItems[4] - Port}
    idDemoGopher.Port := StrToInt ( lvGopherMenu.Selected.SubItems[ 4 ] );
    { SubItems[1] - ItemType }
    GetItem ( lvGopherMenu.Selected.SubItems[1][1] );
...
begin
  ...
  idDemoGopher.Selector := edtSelector.Text;
  {we'll asume that it's a menu when a user types it in}
  GetItem ( IdGopherItem_Directory );
end;
