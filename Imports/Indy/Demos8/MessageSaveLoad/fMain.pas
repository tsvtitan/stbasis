  ...
  IdMessage.SaveToFile(SaveDialog.FileName);
{
  ...
  Rewrite(TF);
  // headers
  IdMessage.Headers.AddStrings(IdMessage.GenerateHeader);
  for i := 0 to IdMessage.Headers.Count - 1 do
    Writeln(TF,idMessage.Headers.Strings[i]);
  Writeln(TF,'');
  for i := 0 to idMessage.Body.Count - 1 do
    WriteLn(TF,IdMessage.Body.Strings[i]);
  WriteLn(TF,'.');
  CloseFile(TF);
  end;
}
...
if OpenDialog.Execute then
  ...
  X := TIdAttachment.Create(IdMessage.MessageParts,OpenDialog.FileName);
  end;
end;
