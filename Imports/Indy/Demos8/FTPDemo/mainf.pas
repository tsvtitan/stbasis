...
//
//  Copyright (C) 2000,2001 Doychin Bondzhev (doychin@dsoft-bg.com)
//
// History:
...
//
//   Jun - 2001 : GUI extensions and fixe to support changes in Log components
//
...
uses
  ...
  Menus,
  {$ENDIF}
  SysUtils, Classes, IdIntercept, IdBaseComponent, IdComponent, IdTCPConnection,
...
type
  TMainForm = class(TForm)
    ...
    procedure IdLogEvent1Sent(ASender: TComponent; const AText,
      AData: String);
{$IFDEF Linux}
    procedure DebugListBoxDrawItem(Sender: TObject; Index: Integer;
      Rect: TRect; State: TOwnerDrawState; var Handled: Boolean);
    procedure DirectoryListBoxDrawItem(Sender: TObject; Index: Integer;
      Rect: TRect; State: TOwnerDrawState; var Handled: Boolean);
...
begin
  ...
  Name := IdFTP1.DirectoryListing.Items[DirectoryListBox.ItemIndex].ItemName;
  if IdFTP1.DirectoryListing.Items[DirectoryListBox.ItemIndex].IsDirectory then begin
    // Change directory
    SetFunctionButtons(false);
    ChageDir(Name);
    SetFunctionButtons(true);
...
begin
  if not IdFTP1.Connected then exit;
  Name := IdFTP1.DirectoryListing.Items[DirectoryListBox.ItemIndex].ItemName;
  if IdFTP1.DirectoryListing.Items[DirectoryListBox.ItemIndex].IsDirectory then try
    SetFunctionButtons(false);
...
begin
  ...
  if DirectoryListBox.ItemIndex > -1 then begin
    if IdFTP1.DirectoryListing.Items[DirectoryListBox.ItemIndex].IsDirectory  then DownloadButton.Caption := 'Change dir'
...
begin
  ...
  if Assigned(IdFTP1.DirectoryListing) and (IdFTP1.DirectoryListing.Count > Index) then begin
    ...
    with IdFTP1.DirectoryListing.Items[Index] do begin
      DirectoryListBox.Canvas.TextOut(Rect.Left, Rect.Top, ItemName);
      R := Rect;
      ...
      DirectoryListBox.Canvas.FillRect(R);
      DirectoryListBox.Canvas.TextOut(R.Left, Rect.Top, FormatDateTime('mm/dd/yyyy hh:mm', Modified));
