...
begin
  ...
  {authentication settings}
   case OutgoingAuthType of
      0: SMTP.AuthenticationType := atNone;
      1: SMTP.AuthenticationType := atLogin; {Simple Login}
   end;
   SMTP.Username := OutgoingServerUser;
   SMTP.Password := OutgoingServerPassword;

   {General setup}
   SMTP.Host := OutgoingServerName;
   SMTP.Port := OutgoingServerPort;
...
begin
   if opdAttachment.Execute then
      begin
         TIdAttachment.Create(IdMsgSend.MessageParts, opdAttachment.FileName);
