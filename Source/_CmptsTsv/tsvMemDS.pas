...
uses Windows, SysUtils, Classes, Controls, DB, DBUtils;
...
{ TMemoryRecord }
  ...
  private
    FMemoryData: TtsvMemoryData;
    FID: Integer;
    FData: Pointer;
    FBlobs: Pointer;
    function GetIndex: Integer;
    procedure SetMemoryData(Value: TtsvMemoryData; UpdateParent: Boolean);
  protected
    procedure SetIndex(Value: Integer); virtual;
  public
    constructor Create(MemoryData: TtsvMemoryData); virtual;
    constructor CreateEx(MemoryData: TtsvMemoryData; UpdateParent: Boolean); virtual;
    destructor Destroy; override;
    property MemoryData: TtsvMemoryData read FMemoryData;
...
constructor TMemoryRecord.Create(MemoryData: TtsvMemoryData);
begin
  CreateEx(MemoryData, True);
end;

constructor TMemoryRecord.CreateEx(MemoryData: TtsvMemoryData;
...
procedure TMemoryRecord.SetMemoryData(Value: TtsvMemoryData; UpdateParent: Boolean);
var
  I: Integer;
  DataSize: Integer;
begin
...
constructor TtsvMemoryData.Create(AOwner: TComponent);
begin
...
destructor TtsvMemoryData.Destroy;
begin
  inherited Destroy;
  FreeIndexList;
  ClearRecords;
...
function TtsvMemoryData.GetCapacity: Integer;
begin
  if FRecords <> nil then Result := FRecords.Capacity
  else Result := 0;
end;

procedure TtsvMemoryData.SetCapacity(Value: Integer);
begin
  if FRecords <> nil then FRecords.Capacity := Value;
end;

function TtsvMemoryData.AddRecord: TMemoryRecord;
begin
  Result := TMemoryRecord.Create(Self);
end;

function TtsvMemoryData.FindRecordID(ID: Integer): TMemoryRecord;
var
  I: Integer;
begin
...
function TtsvMemoryData.InsertRecord(Index: Integer): TMemoryRecord;
begin
  Result := AddRecord;
  Result.Index := Index;
end;

function TtsvMemoryData.GetMemoryRecord(Index: Integer): TMemoryRecord;
begin
...
function TtsvMemoryData.BCDToCurr(BCD: Pointer; var Curr: Currency): Boolean;
begin
  Move(BCD^, Curr, SizeOf(Currency));
  Result := True;
end;

function TtsvMemoryData.CurrToBCD(const Curr: Currency; BCD: Pointer; Precision,
...
procedure TtsvMemoryData.InitFieldDefsFromFields;
var
  I: Integer;
  Offset: Word;
begin
...
function TtsvMemoryData.FindFieldData(Buffer: Pointer; Field: TField): Pointer;
var
  Index: Integer;
begin
{$IFDEF RX_D4}
...
function TtsvMemoryData.CalcRecordSize: Integer;
var
  I: Integer;
begin
  Result := 0;
...
procedure TtsvMemoryData.InitBufferPointers(GetProps: Boolean);
begin
...
procedure TtsvMemoryData.ClearRecords;
begin
...
function TtsvMemoryData.AllocRecordBuffer: PChar;
begin
...
procedure TtsvMemoryData.FreeRecordBuffer(var Buffer: PChar);
begin
  if BlobFieldCount > 0 then
...
procedure TtsvMemoryData.ClearCalcFields(Buffer: PChar);
begin
  FillChar(Buffer[FRecordSize], CalcFieldsSize, 0);
end;

procedure TtsvMemoryData.InternalInitRecord(Buffer: PChar);
var
  I: Integer;
begin
...
procedure TtsvMemoryData.InitRecord(Buffer: PChar);
begin
...
function TtsvMemoryData.GetCurrentRecord(Buffer: PChar): Boolean;
begin
  Result := False;
...
procedure TtsvMemoryData.RecordToBuffer(Rec: TMemoryRecord; Buffer: PChar);
var
  I: Integer;
begin
...
function TtsvMemoryData.GetRecord(Buffer: PChar; GetMode: TGetMode;
...
function TtsvMemoryData.GetRecordSize: Word;
begin
  Result := FRecordSize;
end;

function TtsvMemoryData.GetActiveRecBuf(var RecBuf: PChar): Boolean;
begin
  case State of
    dsBrowse:
...
function TtsvMemoryData.GetFieldData(Field: TField; Buffer: Pointer): Boolean;
var
  RecBuf, Data: PChar;
{$IFDEF RX_D5}
  VarData: Variant;
...
procedure TtsvMemoryData.SetFieldData(Field: TField; Buffer: Pointer);
var
  RecBuf, Data: PChar;
{$IFDEF RX_D5}
  VarData: Variant;
...
procedure TtsvMemoryData.SetFiltered(Value: Boolean);
begin
...
procedure TtsvMemoryData.SetOnFilterRecord(const Value: TFilterRecordEvent);
begin
  if Active then begin
    CheckBrowseMode;
...
function TtsvMemoryData.RecordFilter: Boolean;
var
...
function TtsvMemoryData.GetBlobData(Field: TField; Buffer: PChar): TMemBlobData;
begin
  Result := PMemBlobArray(Buffer + FBlobOfs)[Field.Offset];
end;

procedure TtsvMemoryData.SetBlobData(Field: TField; Buffer: PChar;
  Value: TMemBlobData);
begin
...
procedure TtsvMemoryData.CloseBlob(Field: TField);
begin
...
function TtsvMemoryData.CreateBlobStream(Field: TField; Mode: TBlobStreamMode): TStream;
begin
...
function TtsvMemoryData.BookmarkValid(Bookmark: TBookmark): Boolean;
begin
  Result := FActive and (TBookmarkData(Bookmark^) > Low(Integer)) and
    (TBookmarkData(Bookmark^) <= FLastID);
end;

function TtsvMemoryData.CompareBookmarks(Bookmark1, Bookmark2: TBookmark): Integer;
begin
...
procedure TtsvMemoryData.GetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  Move(PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData, Data^,
    SizeOf(TBookmarkData));
end;

procedure TtsvMemoryData.SetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  Move(Data^, PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData,
    SizeOf(TBookmarkData));
end;

function TtsvMemoryData.GetBookmarkFlag(Buffer: PChar): TBookmarkFlag;
begin
  Result := PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkFlag;
end;

procedure TtsvMemoryData.SetBookmarkFlag(Buffer: PChar; Value: TBookmarkFlag);
begin
  PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkFlag := Value;
end;

procedure TtsvMemoryData.InternalGotoBookmark(Bookmark: TBookmark);
var
  Rec: TMemoryRecord;
  SavePos: Integer;
  Accept: Boolean;
...
procedure TtsvMemoryData.InternalSetToRecord(Buffer: PChar);
begin
  InternalGotoBookmark(@PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData);
end;

procedure TtsvMemoryData.InternalFirst;
begin
  FRecordPos := -1;
end;

procedure TtsvMemoryData.InternalLast;
begin
...
procedure TtsvMemoryData.AssignMemoryRecord(Rec: TMemoryRecord; Buffer: PChar);
var
  I: Integer;
begin
...
procedure TtsvMemoryData.SetMemoryRecordData(Buffer: PChar; Pos: Integer);
var
  Rec: TMemoryRecord;
begin
  if State = dsFilter then Error(SNotEditing);
...
procedure TtsvMemoryData.SetAutoIncFields(Buffer: PChar);
var
  I, Count: Integer;
  Data: PChar;
begin
...
procedure TtsvMemoryData.InternalAddRecord(Buffer: Pointer; Append: Boolean);
var
  RecPos: Integer;
  Rec: TMemoryRecord;
begin
...
procedure TtsvMemoryData.InternalDelete;
var
  Accept: Boolean;
begin
...
procedure TtsvMemoryData.InternalPost;
var
  RecPos: Integer;
begin
...
procedure TtsvMemoryData.OpenCursor(InfoQuery: Boolean);
begin
...
procedure TtsvMemoryData.InternalOpen;
begin
...
procedure TtsvMemoryData.InternalClose;
begin
  ClearRecords;
  FAutoInc := 1;
  BindFields(False);
...
procedure TtsvMemoryData.InternalHandleException;
begin
  Application.HandleException(Self);
end;

procedure TtsvMemoryData.InternalInitFieldDefs;
begin
end;

function TtsvMemoryData.IsCursorOpen: Boolean;
begin
  Result := FActive;
end;
...
function TtsvMemoryData.GetRecordCount: Integer;
begin
  Result := FRecords.Count;
end;

function TtsvMemoryData.GetRecNo: Integer;
begin
  CheckActive;
  UpdateCursorPos;
...
procedure TtsvMemoryData.SetRecNo(Value: Integer);
begin
...
function TtsvMemoryData.IsSequenced: Boolean;
begin
  Result := not Filtered;
end;

function TtsvMemoryData.InternalLocate(const KeyFields: string;
  const KeyValues: Variant; Options: TLocateOptions): Boolean;
var
  fl: TList;
  CurBookmark: string;
...
function TtsvMemoryData.Locate(const KeyFields: string;
  const KeyValues: Variant; Options: TLocateOptions): Boolean;
var
  CurBookmark: string;
begin
...
procedure TtsvMemoryData.EmptyTable;
begin
  if Active then begin
    CheckBrowseMode;
    ClearRecords;
...
procedure TtsvMemoryData.CopyStructure(Source: TDataSet);
...
function TtsvMemoryData.LoadFromDataSet(Source: TDataSet; RecordCount: Integer;
  Mode: TLoadMode): Integer;
var
...
function TtsvMemoryData.SaveToDataSet(Dest: TDataSet; RecordCount: Integer): Integer;
var
  MovedCount: Integer;
begin
  Result := 0;
...
procedure TtsvMemoryData.SortOnFields(const FieldNames: string;
{$IFDEF RX_D4}
  CaseInsensitive: Boolean = True; Descending: Boolean = False);
{$ELSE}
  CaseInsensitive, Descending: Boolean);
...
procedure TtsvMemoryData.QuickSort(L, R: Integer; Compare: TCompareRecords);
var
  I, J: Integer;
  P: TMemoryRecord;
begin
...
function TtsvMemoryData.CompareRecords(Item1, Item2: TMemoryRecord): Integer;
var
  Data1, Data2: PChar;
  F: TField;
  I: Integer;
...
function TtsvMemoryData.GetIsIndexField(Field: TField): Boolean;
begin
...
procedure TtsvMemoryData.CreateIndexList(const FieldNames: string);
var
  Pos: Integer;
  F: TField;
begin
...
procedure TtsvMemoryData.FreeIndexList;
begin
  FIndexList.Free;
  FIndexList := nil;
end;
...
begin
  FMode := Mode;
  FField := Field;
  FDataSet := FField.DataSet as TtsvMemoryData;
