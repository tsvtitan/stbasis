unit URptPms_PriceShareClient3;

interface

uses classes, Forms;

procedure RtpShareRunClient3(Thread: TThread; Form, Options: TForm; sFileName, sTypeOperation: string;
...
uses Windows, sysutils, controls, graphics, IBQuery, IBDatabase, Db, IBCustomDataSet, Excel97,
     UMainUnited, URptPms_Price, URptThread, UPremisesTsvData, UPremisesTsvOptions, tsvInterbase,
     tsvPicture, ClipBrd, Dialogs;
     ...
     try
       ...
       while not qrRegion.Eof do begin
         ...
         sqls:=SQLRbkPms_PremisesRpt+
               ...
               ' order by r.sortnumber, s.name, p.housenumber, p.apartmentnumber, cr.sortnumber, p.price ';
         ...
         if not qrPremises.IsEmpty then begin
           Flag:=false;
           if oldRegionId<>qrRegion.FieldByname('pms_region_id').AsInteger then begin
             Flag:=true;
             inc(curY);
           ...
           oldRegionId:=qrRegion.FieldByname('pms_region_id').AsInteger;

           oldHouse:='';

           NewCount:=qrPremises.RecordCount+1;
           qrPremises.First;
           while not qrPremises.Eof do begin
             if not AnsiSameText(oldHouse,qrPremises.FieldByName('housenumber').AsString) then begin
               inc(NewCount);
             end;
             oldHouse:=qrPremises.FieldByName('housenumber').AsString;
           ...
           oldY:=curY+1;
           Row:=1;
           Data:=VarArrayCreate([1,NewCount,1,ColumnCount],varVariant);
           qrPremises.First;
           while not qrPremises.Eof do begin
             if TRptExcelThreadPms_Price(THread).Terminated then exit;

             if not AnsiSameText(oldHouse,qrPremises.FieldByName('housenumber').AsString) or
                (AnsiSameText(oldHouse,qrPremises.FieldByName('housenumber').AsString) and (Flag)) then begin
               Flag:=false;
               Picture:=TtsvPicture.Create;
               Query:=TIBQuery.Create(nil);
               Ms:=TMemoryStream.Create;
               try
                 Query.Transaction:=tran;
                 Query.Database:=IBDB;
                 Query.SQL.Text:=Format('SELECT IMAGE FROM PMS_IMAGE WHERE PMS_STREET_ID=%d '+
                                        'AND UPPER(HOUSENUMBER)=%s',[qrPremises.FieldByName('pms_street_id').AsInteger,
                                        QuotedStr(qrPremises.FieldByName('housenumber').AsString)]);
                 Query.Active:=true;
                 if Query.Active and not Query.IsEmpty then begin
                   ...
                   if Ms.Size>0 then begin
                     try
                       ...
                       if not Picture.isEmpty then begin
                         inc(curY);
                         inc(Row);
                         ...
                         IW:=IR.Width;
                         if RW>IW then begin
                           IR.Left:=R.Left+(RW/2)-(IW/2);
                         end else begin
                           IR.LockAspectRatio:=0;
                           IR.Left:=R.Left;
                           IR.Width:=RW;
                         end;
                       end;
