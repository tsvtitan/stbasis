unit URptPms_PriceLandAgent;

interface

uses classes, Forms;

procedure RtpLandRunAgent(Thread: TThread; Form, Options: TForm; sFileName, sTypeOperation: string;
...
var
 ...
 qrStation: TIBQuery;
 qrObject: TIBQuery;
 qrPremises,qrAgent,qrDirection,qrCityRegion: TIbQuery;
 sqls: string;
...
// Sh.Range[Sh.Cells[1,1],Sh.Cells[1,1]].Value:=TRptExcelThreadPms_Price(THread).FPlantName;
 ...
 try
   ...
   sqls:='select * from pms_city_region';
     ...
     while not qrAgent.Eof do begin
       ...
       while not qrStation.Eof do begin
         ...
         if not qrPremises.IsEmpty then begin
           ...
           oldY:=curY+1;
           Row:=1;
           Data:=VarArrayCreate([1,qrPremises.RecordCount+2*qrCityRegion.RecordCount,1,ColumnCount],varVariant);
           qrPremises.First;

           OldCityRegion:=qrPremises.FieldByName('cityregionname').AsString;
           Data[Row,1]:=AnsiUpperCase(qrPremises.FieldByName('Direction').AsString+', '+qrPremises.FieldByName('cityregionname').AsString);

           R:=Sh.Range[Sh.Cells[curY+1,1],Sh.Cells[curY+1,ColumnCount]];
           R.Merge;
           R.HorizontalAlignment:=xlCenter;
           R.VerticalAlignment:=xlCenter;
           //R.Font.Size:=8;
          ...
          // Data[Row,1]:='';
           ...
           while not qrPremises.Eof do begin
             if TRptExcelThreadPms_Price(THread).Terminated then exit;
             inc(curY);
             if OldCityRegion<>qrPremises.FieldByName('cityregionname').AsString then
                begin
                Row:=Row+1;
                inc(curY);
           ...
           //R.Font.Size:=8;
             ...
             if IscnppCityRegion then
               Data[Row,IndcnppCityRegion]:=qrPremises.FieldByName('cityregionname').AsString;
             ...
             if IscnppContactClientInfo then
               Data[Row,IndcnppContactClientInfo]:=Trim(TranslateContact(Trim(qrPremises.FieldByName('contact').AsString))
               +', '+qrPremises.FieldByName('email').AsString+', '+qrPremises.FieldByName('clientinfo').AsString)
               +Trim(TranslateContact(Trim(qrPremises.FieldByName('contact2').AsString))
               +', '+qrPremises.FieldByName('email2').AsString+', '+qrPremises.FieldByName('clientinfo2').AsString);
