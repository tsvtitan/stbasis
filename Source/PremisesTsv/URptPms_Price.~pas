...
type
  ...
  TTypePrice=(tpSaleClient,tpSaleAgent,tpSaleInspector,tpLeaseClient,tpLeaseAgent,
              ...
              tpLeaseAgent2,tpShareAgent2,tpShareInspector2,tpShareClient2,tpShareClient3,tpLandClient,tpLandClient1,tpLandAgent);
...
var
  ...
  IscnppSelfFormName,IscnppNote, IscnppAdvertismentNote,
  ...
  //byBart
  IscnppTaxes, IscnppDateTimeInsert,
  IscnppAreaBuilding, IscnppPopulatedPoint, IscnppLandFeature, IscnppExchangeFormula,
  IscnppLocationStatus, IscnppLandMark, IscnppCommunications, IscnppEmail, IscnppLeaseOrSale, IscnppGroundArea,
  IscnppObject, IscnppDirection, IscnppAccessWays, IscnppRemoteness, IscnppCityRegion
  //--
  ...
  IndcnppSelfFormName,IndcnppNote, IndcnppAdvertismentNote,
  ...
  //byBart
  IndcnppTaxes, IndcnppDateTimeInsert,
  IndcnppAreaBuilding, IndcnppPopulatedPoint, IndcnppLandFeature, IndcnppExchangeFormula,
  IndcnppLocationStatus, IndcnppLandMark, IndcnppCommunications, IndcnppEmail, IndcnppLeaseOrSale, IndcnppGroundArea,
  IndcnppObject, IndcnppDirection, IndcnppAccessWays, IndcnppRemoteness, IndcnppCityRegion
...
uses UPremisesTsvCode,comobj,UMainUnited,ActiveX,
     ...
     URptPms_PriceLandClient, URptPms_PriceLandClient1, URptPms_PriceLandAgent;
...
begin
  ...
  if rbLand.Checked then Result:=3;
end;
...
begin
  ...
  if rbLand.Checked then Result:=cmbLand.ItemIndex;
end;
...
begin
  ...
  if rbLand.Checked then Result:='lbxLandStatus';
end;
...
begin
 ...
 try
    ...
    rbLand.Checked:=ReadParam(ClassName,rbLand.Name,rbLand.Checked);
    ...
    cmbLand.ItemIndex:=ReadParam(ClassName,cmbLand.Name,cmbLand.ItemIndex);
...
begin
 ...
 try
    ...
    WriteParam(ClassName,rbLand.Name,rbLand.Checked);
    ...
    WriteParam(ClassName,cmbLand.Name,cmbLand.ItemIndex);
...
{procedure TRptExcelThreadPms_Price.SM;
begin
 showmessage(mess);
end;}
...
var
  ...
  begin
    ...
    if fmParent.rbLand.Checked then Result:=fmParent.rbLand.Caption;
  end;
  ...
  begin
    ...
    if fmParent.rbLand.Checked then Result:=fmParent.cmbLand.Text;
  ...
  begin
   ...
   procedure RptLandRun;
   begin
     case fmParent.cmbLand.ItemIndex of
       0: RtpLandRunClient(Self,fmParent,fmOptions,GetFileName(tpLandClient),GetTypeOperation,UseStyle,FSyncOfficeId);
       1: RtpLandRunClient1(Self,fmParent,fmOptions,GetFileName(tpLandClient1),GetTypeOperation,UseStyle,FSyncOfficeId);
       2: RtpLandRunAgent(Self,fmParent,fmOptions,GetFileName(tpLandAgent),GetTypeOperation,UseStyle,FSyncOfficeId);
     end;
   end;
   ...
   begin
     case fmParent.cmbShare.ItemIndex of
       ...
       4: begin
           {mess:='пидр';
           Synchronize(SM);   }
           RtpShareRunInspector2(Self,fmParent,fmOptions,GetFileName(tpShareInspector2),GetTypeOperation,UseStyle,FSyncOfficeId);
          end; 
...
begin
   ...
   try
     try
       ...
       IscnppAdvertismentNote:=isCC(cnppAdvertismentNote);
       ...
       //byBart
       IscnppTaxes:=isCC(cnppTaxesName);
       IscnppDateTimeInsert:=isCC(cnppDateTimeInsert);

       IscnppAreaBuilding:=isCC(cnppAreaBuilding);
       IscnppPopulatedPoint:=isCC(cnppPopulatedPoint);
       IscnppLandFeature:=isCC(cnppLandFeature);
       IscnppExchangeFormula:=isCC(cnppExchangeFormula);
       IscnppLocationStatus:=isCC(cnppLocationStatus);
       IscnppLandMark:=isCC(cnppLandMark);
       IscnppCommunications:=isCC(cnppCommunications);
       IscnppEmail:=isCC(cnppEmail);
       IscnppLeaseOrSale:=isCC(cnppLeaseOrSale);
       IscnppGroundArea:=isCC(cnppGroundArea);
       IscnppObject:=isCC(cnppObject);
       IscnppDirection:=isCC(cnppDirection);
       IscnppAccessWays:=isCC(cnppAccessWays);
       IscnppRemoteness:=isCC(cnppRemoteness);

       IscnppCityRegion:=isCC(cnppCityRegion);
       //--
       ...
       IndcnppAdvertismentNote:=GCP(cnppAdvertismentNote);
       ...
       //byBart
       IndcnppTaxes:=GCP(cnppTaxesName);
       IndcnppDateTimeInsert:=GCP(cnppDateTimeInsert);

       IndcnppAreaBuilding:=GCP(cnppAreaBuilding);
       IndcnppPopulatedPoint:=GCP(cnppPopulatedPoint);
       IndcnppLandFeature:=GCP(cnppLandFeature);
       IndcnppExchangeFormula:=GCP(cnppExchangeFormula);
       IndcnppLocationStatus:=GCP(cnppLocationStatus);
       IndcnppLandMark:=GCP(cnppLandMark);
       IndcnppCommunications:=GCP(cnppCommunications);
       IndcnppEmail:=GCP(cnppEmail);
       IndcnppLeaseOrSale:=GCP(cnppLeaseOrSale);
       IndcnppGroundArea:=GCP(cnppGroundArea);
       IndcnppObject:=GCP(cnppObject);
       IndcnppDirection:=GCP(cnppDirection);
       IndcnppAccessWays:=GCP(cnppAccessWays);
       IndcnppRemoteness:=GCP(cnppRemoteness);
       IndcnppCityRegion:=GCP(cnppCityRegion);
       //---
       ...
       if fmParent.rbLand.Checked then RptLandRun;
     except
      {$IFDEF DEBUG}
...
begin
  ...
  cmbLand.Enabled:=rbLand.Checked and isPerm;
  cmbLand.Color:=iff(rbLand.Checked and isPerm,clWindow,clBtnFace);
...
begin
  ...
  if rbLand.Checked and (cmbLand.ItemIndex in [0,1]) then begin
    Result:='crg.sortnumber, r.name, p.price';
  end else
  if rbLand.Checked and (cmbLand.ItemIndex in [2]) then begin
    Result:='crg.sortnumber, r.name, obj.sortnumber, p.price';
  end else begin
...
begin
 ...
 case TypePrice of
   ...
   tpLandClient: Result:=ConstResNamePms_Price_LandClient;
   tpLandClient1: Result:=ConstResNamePms_Price_LandClient1;
   tpLandAgent: Result:=ConstResNamePms_Price_LandAgent;
 end;
end;
...
procedure TfmRptPms_Price.FillColumns(Default: Boolean=false);
  ...
  begin
    ...
    case Index of
      0: begin // Клиентский
        ...
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppAdvertismentNote),TObject(Pointer(cnppAdvertismentNote)));
        ...
        //byBart
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppTaxesName),TObject(Pointer(cnppTaxesName)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppDateTimeInsert),TObject(Pointer(cnppDateTimeInsert)));
      end;
      1:begin // Агентский
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppDateArrivals),TObject(Pointer(cnppDateArrivals)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppCountRoomName),TObject(Pointer(cnppCountRoomName)));
  ...
  begin
    ...
    case Index of
      0: begin // Клиентский
        ...
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppAreaBuilding),TObject(Pointer(cnppAreaBuilding)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppPopulatedPoint),TObject(Pointer(cnppPopulatedPoint)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppLandFeature),TObject(Pointer(cnppLandFeature)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppExchangeFormula),TObject(Pointer(cnppExchangeFormula)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppLocationStatus),TObject(Pointer(cnppLocationStatus)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppLandMark),TObject(Pointer(cnppLandMark)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppEmail),TObject(Pointer(cnppEmail)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppCommunications),TObject(Pointer(cnppCommunications)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppLeaseOrSale),TObject(Pointer(cnppLeaseOrSale)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppGroundArea),TObject(Pointer(cnppGroundArea)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppObject),TObject(Pointer(cnppObject)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppDirection),TObject(Pointer(cnppDirection)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppRemoteness),TObject(Pointer(cnppRemoteness)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppAccessWays),TObject(Pointer(cnppAccessWays)));
      ...
      2:begin // Агентский
        ...
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppRemoteness),TObject(Pointer(cnppRemoteness)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppAccessWays),TObject(Pointer(cnppAccessWays)));
        lbxColumns.Items.AddObject(GetColumnPremisesName(cnppCityRegion),TObject(Pointer(cnppCityRegion)));
  ...
  begin
    ...
    if rbLand.Checked then begin
      Index:=GetTypePriceIndex;
      case Index of
        0: Result:=tpLandClient;
        1: Result:=tpLandClient1;
        2: Result:=tpLandAgent;
      end;
    end;
...
begin
  ...
  try
    ...
    if rbLand.Checked then FillLandColumns;
    UnCheckAll(lbxColumns,false);
    LoadFromResource;
    if not Default then
      LoadColumns;
...
begin
  ...
  if rbLand.Checked then Result:='lbxLandColumns';
end;
