...
uses
  Forms, Controls, Messages, SysUtils, Consts, Windows,Graphics,Classes,
  ExtCtrls, Math, StdCtrls, Menus, ELSConsts
  {$IFDEF VER140}, RTLConsts {$ENDIF}, Clipbrd, ELControls, tsvHint,
  buttons, DsgnIntf, tsvDesignCore, Dialogs;
...
type
  ...
  TELAlignSelection = (asLeft, asRight, asTop, asBottom, asHorCenter, asHorInWin,
  ...
  private
   ...
   procedure Change;
    ...
    function GetDefaultComponent: TComponent;
  public
    constructor Create(AOwnerDesigner: TELCustomDesigner);
    destructor Destroy; override;
    function Add(AControl: TControl): Integer;
    function AddComponent(AComponent: TComponent): Integer;
  ...
  TELDesignerOnComponentHintEvent = procedure(Sender: TObject;
    AComponent: TComponent; var AHint: string) of object;
  ...
  TOnGetDesignComponentGlyph=procedure(Sender: TObject;
                               AClass: TClass; var Glyph: TBitmap)of object;
  TOnSetDesignComponent=procedure(Sender: TObject; AComponent: TComponent) of object;
  ...
  private
    ...
    FOnComponentHint: TELDesignerOnComponentHintEvent;
    ...
    FDesignComponentWidth: Integer;
    FOnGetDesignComponentGlyph: TOnGetDesignComponentGlyph;
    FOnSetDesignComponent: TOnSetDesignComponent;
    FSizeHandle: Integer;
    FVisibleComponentCaption: Boolean;

    procedure SetVisibleComponentCaption(Value: Boolean);
    procedure SetSizeHandle(Value: Integer);
    procedure SetDesignComponent(Sender: TObject; AComponent: TComponent);
    procedure SetDesignComponentWidth(Value: Integer);
    ...
    procedure BringToFrontDesignComponent;
  protected
    ...
    procedure ControlInserting(var AComponentClass: TComponentClass); virtual;
    procedure ControlInserted; virtual;
    procedure DoNotification(AnObject: TPersistent; Operation: TOperation); virtual;
    procedure ComponentHint(AComponent: TComponent; var AHint: string); virtual;
    ...
    property OnComponentHint: TELDesignerOnComponentHintEvent read FOnComponentHint write FOnComponentHint;
    ...
    property DesignComponentWidth: Integer read FDesignComponentWidth write SetDesignComponentWidth;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    ...
    procedure Copy;
    procedure Paste;
    function CreateDesignComponent(AComponent: TComponent): TControl;
    function GetDesignComponent(AComponent: TComponent): TControl;
    function AssignDesignComponent(AComponent: TComponent): TControl;
    procedure AssignDesignComponents;
    property OnSetDesignComponent: TOnSetDesignComponent read FOnSetDesignComponent
                                                         write FOnSetDesignComponent;
    property OnGetDesignComponentGlyph: TOnGetDesignComponentGlyph read FOnGetDesignComponentGlyph
                                                                   write FOnGetDesignComponentGlyph;
  ...
  TDesignComponent=class(TCustomControl)
  end;
...
//    property WordWrap;
  ...
  TLabelPosition = (lpAbove, lpBelow, lpLeft, lpRight, lpNoneVisible);
  
  TDesignComponentEx=class(TDesignComponent)
  private
...
{  TDesignComponent=class(TSpeedButton)
  private
    FComponent: TComponent;
  public
    constructor Create(AOwner: TComponent);override;
    ...
    destructor Destroy;override;
    procedure Paint; override;
    procedure PaintTo(HDC: THandle; X,Y: Integer);
...
begin
  ...
  if Assigned(AOwner) then
    if AOwner is TDesignComponentEx then
      if TDesignComponentEx(AOwner).FComponent<>nil then
        Caption := TDesignComponentEx(AOwner).FComponent.Name;
end;

procedure TBoundLabel.AdjustSize;
begin
  inherited AdjustSize;
  if Owner is TDesignComponentEx then
    with Owner as TDesignComponentEx do
...
constructor TDesignComponentEx.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TDesignComponentEx.CreateParams(var Params: TCreateParams);
begin
  inherited;
  Params.Style:=WS_CHILD or WS_VISIBLE or WS_CLIPSIBLINGS or WS_EX_TOPMOST;
end;

procedure TDesignComponentEx.CreateWnd;
begin
  inherited;
...
destructor TDesignComponentEx.Destroy;
begin
  FEditLabel.Free;
  FGlyph.Free;
  inherited Destroy;
...
procedure TDesignComponentEx.SetGlyph(Value: TBitmap);
begin
  FGlyph.Assign(Value);
end;

procedure TDesignComponentEx.SetWidth(Value: Integer);
begin
...
procedure TDesignComponentEx.Paint;
var
  FDI: LongInt;
  ARect: TRect;
  dx,dy: Integer;
begin
  ...
  if not Glyph.Empty then begin
   dx:=Width div 2 - Glyph.Width div 2;
   dy:=Height div 2 - Glyph.Height div 2;
   Glyph.TransparentColor:=Glyph.Canvas.Pixels[0,0];
   Glyph.Transparent:=true;
   Canvas.Draw(dx,dy,Glyph);
  end;
  Frame3D(Canvas,ARect,clBtnHighlight,clBtnShadow,1);
end;

procedure TDesignComponentEx.CMBidimodechanged(var Message: TMessage);
begin
  inherited;
  FEditLabel.BiDiMode := BiDiMode;
end;

procedure TDesignComponentEx.CMEnabledchanged(var Message: TMessage);
begin
  inherited;
  FEditLabel.Enabled := Enabled;
end;

procedure TDesignComponentEx.CMVisiblechanged(var Message: TMessage);
begin
  inherited;
end;

procedure TDesignComponentEx.Notification(AComponent: TComponent;
...
procedure TDesignComponentEx.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  inherited SetBounds(ALeft, ATop, AWidth, AHeight);
  SetLabelPosition(FLabelPosition);
end;

procedure TDesignComponentEx.SetLabelPosition(const Value: TLabelPosition);
var
  P: TPoint;
begin
  ...
  case Value of
    lpAbove: P := Point(Left+Width div 2 -FEditLabel.Width div 2, Top - FEditLabel.Height - FLabelSpacing);
    lpBelow: P := Point(Left+Width div 2 -FEditLabel.Width div 2, Top + Height + FLabelSpacing);
...
procedure TDesignComponentEx.SetLabelSpacing(const Value: Integer);
begin
  FLabelSpacing := Value;
  SetLabelPosition(FLabelPosition);
end;

procedure TDesignComponentEx.SetParent(AParent: TWinControl);
begin
  inherited SetParent(AParent);
  if FEditLabel = nil then exit;
  FEditLabel.Parent := AParent;
  FEditLabel.Visible := FVisibleCaption;
end;

procedure TDesignComponentEx.SetupInternalLabel;
begin
  ...
  FEditLabel.Visible:=FVisibleCaption;
  FEditLabel.AutoSize:=true;
  FEditLabel.BorderStyle:=sbsNone;
end;

procedure TDesignComponentEx.SetComponent(Value: TComponent);
begin
  if Value<>FComponent then begin
    FComponent:=Value;
    if Value<>nil then begin
      FEditLabel.Caption:=Value.Name;
    end;
  end;
end;

procedure TDesignComponentEx.SetVisibleCaption(Value: Boolean);
begin
  if Value<>FVisibleCaption then begin
   FVisibleCaption:=Value;
   FEditLabel.Visible:=Value;
  end; 
end;
...
begin
  if DHintHook <> 0 then UnhookWindowsHookEx(DHintHook);
  DHintHook := 0;
end;
...
begin
  if FRoot is TCustomForm then Result := TCustomForm(FRoot)
    else Result := nil;
end;
...
begin
  ...
  if not FIsInDrawMode then FHint.CheckHideMessageProc(0, Message);
  case Message.Msg of
    CM_DRAG:
    begin
      case TCMDrag(Message).DragMessage of
        dmDragEnter, dmDragLeave, dmDragMove, dmDragDrop:
          ...
          begin
            ...
            if LTarget <> nil then LTarget := GetDesignControl(LTarget);
            if (LTarget <> nil) and (DragRec.Source <> nil) then
            begin
              S := DragRec.Source;
              IsDockOp := S is TDragDockObject;
              if not IsDockOp then S := (S as TDragControlObject).Control;
              if S <> nil then
    ...
    begin
      ...
      if LI <> -1 then FSelCtrls.UpdateControl(LI);
    end;
    WM_SETFOCUS:
    ...
    WM_CLOSE:
    begin
      if (Sender = FRoot) and (FRoot is TCustomForm) then _FormClose;
      Result := True;
    end;
  end;
end;
...
{  for i:=0 to FRoot.ComponentCount-1 do begin
    if FRoot.Components[i] is TDesignComponent then begin
      TDesignComponent(FRoot.Components[i]).PaintTo(
           FForm.Canvas.Handle,TDesignComponent(FRoot.Components[i]).left,
           TDesignComponent(FRoot.Components[i]).Top);
    end;
  end;}
end;
...
begin
  ...
  begin
    ...
    if LIsValidName then FDesigner.ValidateName(NewName, LIsValidName);
...
function TDEng.MouseMessage(Sender: TControl; const  Message: TMessage): Boolean;
  ...
  begin
    ...
    if _FindVisibleScreenRect(AControlToDrawOn, LR) then ClipCursor(@LR);
  ...
  begin
    try
      LP := _GetVirtualCursorPos(FInitCursorPos);
      if ACancel then LFlag := ddpfCancelRemoveAndDispose
        else LFlag := pfOkRemoveAndDispose;
      try
        FInitDrawProc(FCanvas, FOldDrawInfo, LP, LFlag);
      finally
        if GetCaptureControl = FRoot then SetCaptureControl(nil);
        ClipCursor(nil);
        FCanvas.Free;
        FCanvas := nil;
      end;
...
begin
  ...
  case Message.Msg of
      ...
      begin
        ...
        begin
          ...
          if not (lmNoInsertIn in GetLockMode(LContainer)) and
            (LContainer <> nil) then
          begin
            FDesigner.ControlInserting(FInsertingComponentClass);
            ...
            begin
              ...
              try
                ...
                begin
                  ...
                  begin
                    for LI := LControls.Count - 1downto 0 do
                  ...
                  else FSelCtrls.SelectControls(LControls);
                end
                else FSelCtrls.SelectControls(LControls);
              finally
                LControls.Free;
    ...
    WM_MOUSEMOVE:
      ...
      begin
        ...
        begin
          if (Sender = FRoot) or (Sender = FForm) or not Result then
            ...
            begin
              ...
              begin
                ...
                FDesigner.ComponentHint(Sender, LS);
                if LS <> '' then
                begin
                  FHint.Caption := LS;
          ...
          if Result then FHintControl := Sender else FHintControl := nil;
        end;
      end;
    WM_LBUTTONUP:
      if FIsInDrawMode then
    ...
    WM_LBUTTONDBLCLK:
      if not FIsInDrawMode then FDesigner.DblClick;
    DESIGNER_CANCEL:
      if FIsInDrawMode then
      begin
  ...
  case AFlag of
    ...
    begin
      if AOldDataPtr = nil then
        New(PPointsRect(Result)) else Result := AOldDataPtr;
      _FillData(Result);
      if AOldDataPtr = nil then
    ...
    pfOkRemoveAndDispose:
      try
        ...
        try
          ...
          if FSelCtrls.Count > 1 then FSelCtrls.Remove(FRoot);
        finally
          FSelCtrls.EndUpdate;
        end;
      finally
...
type
  ...
  function _InitData: PDataRec;
    ...
    begin
      ...
      if AData.MinLeftTop.x > LR.Left then AData.MinLeftTop.x := LR.Left;
      if AData.MinLeftTop.y > LR.Top then AData.MinLeftTop.y := LR.Top;
    end;
  ...
  case AFlag of
    ...
    pfOkRemoveAndDispose:
      try
        ...
        begin
          with LDrawRect do PDataRec(AOldDataPtr).Control.SetBounds(Left, Top,
      ...
      case TWMKeyDown(Message).CharCode of
        VK_ESCAPE:
          ...
          begin
            ...
            if LControl = nil then LControl := FRoot;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        while True do
        begin
          if LI > FSelCtrls.Count - 1 then LI := FSelCtrls.Count - 1;
          if LI < 0 then Break;
          if FSelCtrls[LI] <> FRoot then FSelCtrls[LI].Free;
          Dec(LI);
        end;
      end
...
type
  ...
  function CreateLInsertingControl: TControl;
  var
    ct: TComponent;
  begin
    if isClassParent(FInsertingComponentClass,TControl) then begin
      Result:=TControl(FInsertingComponentClass.Create(FRoot));
    end else begin
      ct:=FInsertingComponentClass.Create(FRoot);
      ct.Name:=UniqueName(ct.ClassName);
      Result:=FDesigner.CreateDesignComponent(ct);
    end;
  end;

  procedure SetLocalParent(ct: TControl);
  begin
    if ct is TDesignComponentEx then
     ct.Parent:=FRoot
    else
     ct.Parent:=FCanvas.WinControl;
  end;

  procedure SetLocalBounds(ct: TControl; LDrawRect: TRect);
  var
    pt1: TPoint;
  begin
    if ct is TDesignComponentEx then begin
      pt1:=Point(LDrawRect.Left,LDrawRect.Top);
      pt1:=FCanvas.WinControl.ClientToScreen(pt1);
      pt1:=ct.Parent.ScreenToClient(pt1);
      LDrawRect:=Rect(pt1.x,pt1.y,LDrawRect.Right,LDrawRect.Bottom);
    end;  

      with LDrawRect do
         if (Right - Left > 1) or (Bottom - Top > 1) then
           ct.SetBounds(Left, Top, Right - Left, Bottom - Top)
         else
           ct.SetBounds(Left, Top, ct.Width, ct.Height);
  end;
  ...
  case AFlag of
    ...
    pfOkRemoveAndDispose:       
      try
        LDrawRect := _CalcDrawRect(AOldDataPtr);
        FHint.Hide;
        if PDataRec(AOldDataPtr)^.Drawn then _DoXORDraw(LDrawRect);
        LInsertingControl :=CreateLInsertingControl;
        try
          with LInsertingControl do
          begin
            SetLocalParent(LInsertingControl);// <-- Here may be exception
            SetLocalBounds(LInsertingControl,LDrawRect);
            LName := UniqueName(LInsertingControl.ClassName);
...
begin
  ...
  try
    ...
    for LI := 0 to AControls.Count - 1 do
      ...
      begin
        LW.WriteSignature;
        if not (TComponent(AControls[LI]) is TDesignComponentEx) then
         LW.WriteComponent(AControls[LI])
        else
         LW.WriteComponent(TDesignComponentEx(AControls[LI]).FComponent);
      end;
    LW.WriteListEnd;
  finally
    LW.Free;
...
procedure TDEng.ReaderReadComponent(Component: TComponent);
  ...
  function GetLocalControl: TControl;
  begin
    if Component is TControl then begin
      Result:=TControl(Component);
    end else begin
      Result:=FDesigner.AssignDesignComponent(Component);
    end;
  end;
  ...
  begin
    ...
    for LI := 0 to AParent.ControlCount - 1 do
      if AParent.Controls[LI] <> GetLocalControl then
...
begin
  ...
  if IsDesignControl(AControl) then
    ...
    begin
      ...
      FSelCtrls.Add(AControl);
      Fdesigner.SetDesignComponent(FDesigner,Component);
    end;
end;
...
begin
  ...
  if LI <> -1 then Name := FReaderNames.DestName(LI);
end;
...
begin
  ...
  if FForm.ParentWindow <> 0 then ShowWindow(FForm.Handle, SW_SHOW);
end;
...
begin
  if IsUsed and HandleAllocated then FForm.ParentWindow := Handle
    else FForm.ParentWindow := 0;
end;
...
begin
  ...
  begin
    ...
    if DHintWindow.IsHintMsg(LMsg) then Hide;
  end;
end;
...
begin
  ...
  begin
    DHintWindow := TNewHintWindow.Create(nil);
...
begin
  ...
  if DHintWindowShower = Self then DHintWindowShower := nil;
  inherited;
end;
...
procedure TDSelCtrlItemPoint.SetSize(Value: Integer);
begin
...
function TDSelCtrlItemPoint.GetSize: Integer;
begin
  Result:=Width;
end;
...
begin
  ...
  case FPos of
    ppTopLeft, ppTop, ppTopRight:
      Top := FSelCtrl.Control.Top -
        (Width div 2) * Ord(FSelCtrl.Mode = imSizeable);
    ppRight, ppLeft:
      Top := FSelCtrl.Control.Top +
        FSelCtrl.Control.Height div 2 - (Width div 2);
    ppBottomRight, ppBottom, ppBottomLeft:
      Top := FSelCtrl.Control.Top + FSelCtrl.Control.Height - Width +
        (Width div 2) * Ord(FSelCtrl.Mode = imSizeable);
  end;
  case FPos of
    ...
    ppTop, ppBottom:
      Left := FSelCtrl.Control.Left +
        FSelCtrl.Control.Width div 2 - (Width div 2);
    ppTopRight, ppRight, ppBottomRight:
      Left := FSelCtrl.Control.Left + FSelCtrl.Control.Width - Width +
        (Width div 2) * Ord(FSelCtrl.Mode = imSizeable);
  end;
  ...
  begin
    ...
    if lmNoMove in LLockMode then Color := LDesigner.LockedHandleClr;
    if lmNoResize in LLockMode then FBorderColor := LDesigner.LockedHandleClr;
  end
  else
  begin
    if FSelCtrl.Mode = imSizeable then
  ...
  BringToFront;
  if (FSelCtrl.Mode = imSizeable) and not (lmNoResize in LLockMode)
    then Cursor := HANDLECURSORS[FPos]
...
begin
  ...
  FControl := AControl;
  for LI := ppTopLeft to ppLeft do begin
    FPoints[LI] := TDSelCtrlItemPoint.Create(Self, LI);
    FPoints[LI].SetSize(FSelCtrls.FDesigner.FSizeHandle);
    FPoints[LI].Update;
  end;
...
begin
  ...
  if LJ <> -1 then FSelCtrls.DeleteItem(LJ);
  for LI := ppTopLeft to ppLeft do FPoints[LI].Free;
  inherited;
end;
...
var
  LI: TDSelCtrlItemPointPos;
begin
  for LI := ppTopLeft to ppLeft do FPoints[LI].Update;
end;
...
begin
  if FVisible then
  begin
    if Count = 1 then _SetHandlesMode(imSizeable)
      else _SetHandlesMode(imMultySelect);
  end else _SetHandlesMode(imNone);
end;
...
var
  LI: Integer;
begin
  for LI := 0 to Count - 1 do TDSelCtrlItem(FItems[LI]).Update;
end;
...
function TELDesignerSelectedControls.AddComponent(AComponent: TComponent): Integer;
var
  AControl: TControl;
begin
  Result:=-1;
  if AComponent is TControl then begin
    if not (AComponent.Owner is TDesignComponentEx) then begin
      AControl:=TControl(AComponent);
      Result:=Add(AControl);
    end else begin
      AControl:=TControl(TDesignComponentEx(AComponent.Owner).Component);
      if AControl<>nil then
        Result:=Add(AControl);
    end;
  end else begin
    AControl:=FDesigner.GetDesignComponent(AComponent);
    if AControl<>nil then
    Result:=Add(AControl);
  end;
end;
...
begin
  if FRootWinControl = Value then Exit;
...
begin
  LI := IndexOf(AControl);
  if LI <> -1 then begin
{    if AControl is TDesignComponentEx then begin fuck
      TDesignComponentEx(AControl).free;
    end;}
    Delete(LI);
  end;  
end;
...
begin
  if not FDestroying then FDesigner.CheckActive(True);
  BeginUpdate;
  try
    while (Count > 0) and (Items[0] <> AControl) do Delete(0);
...
procedure TELDesignerSelectedControls.AlignSelection(const AAlign: TELAlignSelection);
var
  Source: TControl;

  function GetLocalControl(Index: Integer): TControl;
  begin
    Result:=nil;
  ...
  procedure GetBoundsFromControls(var minT,maxB,minL,maxR: TControl);
  var
    i: Integer;
    ct: TControl;
  begin
    for i:=0 to Count-1 do begin
      ct:=Items[i];
      if ct.Parent=Source.Parent then begin
        if minT=nil then minT:=ct;
        if maxB=nil then maxB:=ct;
        if minL=nil then minL:=ct;
        if maxR=nil then maxR:=ct;

        if ct.Top<minT.Top then minT:=ct;
        if (ct.Top+ct.Height)>(maxB.Top+maxB.Height) then maxB:=ct;
        if ct.Left<minL.Left then minL:=ct;
        if (ct.Left+ct.Width)>(maxR.Left+maxR.Width) then maxR:=ct; 
      end;
    end;
  end;

  procedure SetLocalHorSpace;
  var
    minT,maxB,minL,maxR: TControl;
    i: Integer;
    ct,oldct: TControl;
    dx: Integer;
  begin
    ...
    minL:=nil;
    maxR:=nil;
    GetBoundsFromControls(minT,maxB,minL,maxR);
    if minL=nil then exit;
    if maxR=nil then exit;
    dx:=(maxR.Left-minL.Left) div (count-1);
    oldct:=minL;
    for i:=0 to Count-1 do begin
      ct:=Items[i];
      if ct.Parent=Source.Parent then begin
        if (ct<>minL)and(ct<>maxR) then begin
          ct.Left:=oldct.Left+dx;
        end;
      end;
      oldct:=ct;
    end;
  ...
  begin
    ...
    maxR:=nil;
    GetBoundsFromControls(minT,maxB,minL,maxR);
    if minT=nil then exit;
    if maxB=nil then exit;
    dy:=(maxB.Top-minT.Top) div (count-1);
    oldct:=minT;
    for i:=0 to Count-1 do begin
      ct:=Items[i];
      if ct.Parent=Source.Parent then begin
        if (ct<>minT)and(ct<>maxB) then begin
          ct.Top:=oldct.Top+dy;
        end;
      end;
      oldct:=ct;
...
var
  ...
  tmpCt: TControl;
begin
  Source:=GetLocalControl(0);
  for i := 0 to Count - 1 do begin
    tmpct:=Items[i];
    if (tmpct<>nil)and(tmpct.Parent=Source.Parent) then begin
     with tmpct do
      case AAlign of
        asLeft:
        ...
        asHorCenter:begin
          Left:=(Source.Left+Source.Width div 2)- Width div 2;
        end;
        asVerCenter:begin
          Top:=(Source.Top+Source.Height div 2)- Height div 2;
        end;
      end;    { case }
    end;
  end;
...
begin
  FWinControl := Value;
  if Value <> nil then FHiddenControl.ParentWindow := Value.Handle
    else FHiddenControl.ParentWindow := 0;
  Handle := 0;
end;
...
begin
  ...
  SetDesignComponent(Self,FSelectedControls.DefaultComponent);
end;
...
var
  LS: string;
begin
  if AIsActiveNeeded then LS := 'Designer mast be active'
    else LS := 'Designer mast be inactive';
  if Active <> AIsActiveNeeded then raise EELDesigner.Create(LS);
end;

procedure TELCustomDesigner.ComponentHint(AComponent: TComponent; var AHint: string);
begin
  if AComponent is TDesignComponentEx then begin
    AComponent:=TDesignComponentEx(AComponent).FComponent;
  end;
  AHint:=AComponent.Name+': '+AComponent.ClassName;
  if Assigned(OnComponentHint) then OnComponentHint(Self, AComponent, AHint);
end;

procedure TELCustomDesigner.ControlInserted;
begin
  BringToFrontDesignComponent;
  if Assigned(OnControlInserted) then OnControlInserted(Self);
end;

procedure TELCustomDesigner.ControlInserting(
  var AComponentClass: TComponentClass);
begin
  if Assigned(OnControlInserting) then OnControlInserting(Self, AComponentClass);
end;

procedure TELCustomDesigner.SetDesignComponentWidth(Value: Integer);
begin
  if Value<>FDesignComponentWidth then begin
    FDesignComponentWidth:=Value;
  end;
end;

procedure TELCustomDesigner.SetDesignComponent(Sender: TObject; AComponent: TComponent);
begin
  if Assigned(FOnSetDesignComponent) then FOnSetDesignComponent(Sender,AComponent);
end;
...
begin
  ...
  FSizeHandle:=HANDLESIZE;
  FVisibleComponentCaption:=false;
end;
...
procedure TELCustomDesigner.SetVisibleComponentCaption(Value: Boolean);
var
  i: Integer;
  ct: TComponent;
begin
  FVisibleComponentCaption:=Value;
  for i:=0 to FDesignControl.ComponentCount-1 do begin
    ct:=FDesignControl.Components[i];
    if (ct is TDesignComponentEx) then begin
      TDesignComponentEx(ct).VisibleCaption:=Value;
    end;
  end;
end;

procedure TELCustomDesigner.SetSizeHandle(Value: Integer);
var
  i: Integer;
  j: TDSelCtrlItemPointPos;
begin
  if FSizeHandle<>Value then begin
    FSizeHandle:=Value;
    for i:=0 to FSelectedControls.FItems.Count-1 do begin
      for j:=Low(TDSelCtrlItemPointPos) to High(TDSelCtrlItemPointPos) do begin
        TDSelCtrlItem(FSelectedControls.FItems.Items[i]).FPoints[j].SetSize(Value);
        TDSelCtrlItem(FSelectedControls.FItems.Items[i]).FPoints[j].Update;
      end;
    end;
  end;
end;
...
begin
  if Active then TDEng(FEng).GridParamsChanged;
end;
...
function TELCustomDesigner.GetDesignComponent(AComponent: TComponent): TControl;
var
  i: Integer;
  ct: TComponent;
begin
  ...
  for i:=0 to FDesignControl.ComponentCount-1 do begin
    ct:=FDesignControl.Components[i];
    if (ct is TDesignComponentEx) then begin
      if TDesignComponentEx(ct).Component=AComponent then begin
        Result:=TDesignComponentEx(ct);  
        exit;
      end; 
    end;
  end;
...
function InVisibleComponent(AClass: TClass): Boolean;
begin
  Result:=false;
  if isClassParent(AClass,TmenuItem) then exit;
  Result:=true;
end;

function TELCustomDesigner.CreateDesignComponent(AComponent: TComponent): TControl;
var
  dc: TDesignComponentEx;
  gl: TBitmap;
begin
  Result:=nil;
  if not InVisibleComponent(AComponent.ClassType) then exit;
  dc:=TDesignComponentEx.Create(FDesignControl);
  dc.Component:=AComponent;
  dc.SetWidth(FDesignComponentWidth);
  gl:=nil;
  if Assigned(FOnGetDesignComponentGlyph) then
    OnGetDesignComponentGlyph(Self,AComponent.ClassType,gl);
  dc.Glyph.Assign(gl);
  dc.VisibleCaption:=VisibleComponentCaption;
  Result:=dc;
end;

function TELCustomDesigner.AssignDesignComponent(AComponent: TComponent): TControl;
var
  dc: TDesignComponentEx;
begin
  Result:=nil;
  if not (AComponent is TControl) then begin
    dc:=TDesignComponentEx(GetDesignComponent(AComponent));
    if dc=nil then
     dc:=TDesignComponentEx(CreateDesignComponent(AComponent));
    if dc=nil then exit;
    dc.Parent:=FDesignControl;
    dc.Left:=Longrec(AComponent.DesignInfo).Lo;
    dc.Top:=Longrec(AComponent.DesignInfo).Hi;
    Result:=dc;
  end;
end;

procedure TELCustomDesigner.AssignDesignComponents;
var
  i: Integer;
  ct: TComponent;
begin
...
procedure TELCustomDesigner.Notification(AComponent: TComponent;
  ...
  function GetLastComponent(DesignParent: TWinControl): TComponent;
  var
    i: Integer;
  begin
    Result:=nil;
    if DesignParent=nil then exit;
    if DesignParent.ControlCount=0 then exit;
    for i:=DesignParent.ControlCount-1 downto 0 do begin
      if not (DesignParent.Controls[i].Owner is TDesignComponentEx) then begin
        Result:=DesignParent.Controls[i];
        exit;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if DesignPanel.IsUsed then Active := False;
    ...
    if AComponent is TControl then begin
      ...
      if AComponent is TDesignComponentEx then begin
        DoNotification(TDesignComponentEx(AComponent).Component,opRemove);
        TDesignComponentEx(AComponent).Component.Free;
        SelectedControls.Remove(TControl(AComponent));
        ct:=GetLastComponent(DesignControl);
        if ct<>nil then begin
         SelectedControls.AddComponent(ct);
        end; 
      end else begin
        SelectedControls.Remove(TControl(AComponent));
        ct:=GetLastComponent(DesignControl);
...
begin
  CheckActive(False);
  if FDesignControl <> nil then FDesignControl.RemoveFreeNotification(Self);
  FDesignControl := Value;
  if FDesignControl <> nil then FDesignControl.FreeNotification(Self);
end;
...
begin
  if Assigned(OnValidateName) then OnValidateName(Self, AName, AIsValidName);
end;

procedure TELCustomDesigner.ChangeDesignPanel(
  const Value: TELCustomDesignPanel);
begin
  if FDesignPanel <> nil then FDesignPanel.RemoveFreeNotification(Self);
  FDesignPanel := Value;
  if FDesignPanel <> nil then FDesignPanel.FreeNotification(Self);
end;
...
begin
  GetCursorPos(LPt);
  LHandled := False;
  if Assigned(OnContextPopup) then OnContextPopup(Self, LPt, LHandled);
...
begin
  ...
  try
    for LI := Count - 1 downto 0 do
      if AControls.IndexOf(Items[LI]) = -1 then Delete(LI);
    for LI := 0 to AControls.Count - 1 do
      Add(AControls[LI]);
  finally
    EndUpdate;
      ...
      try
        ...
        try
          ...
          if SelectedControls.Count > 0 then
            ...
            LParent := nil;
          if LParent = nil then LParent := TDEng(FEng).FRoot;
...
begin
  if FClipboardFormat = Value then Exit;
...
procedure TELCustomDesigner.DoNotification(AnObject: TPersistent;
  Operation: TOperation);
begin
  if Assigned(OnNotification) then OnNotification(Self, AnObject, Operation);
end;
...
begin
  ...
  try
    for LI := Count - 1 downto 0 do
      if Items[LI].Parent <> AParent then Delete(LI);
  finally
    EndUpdate;
  end;
end;
  ...
  begin
    ...
    for LI := 0 to SelectedControls.Count - 1 do
      if
       ...
       and (SelectedControls[LI] <> TDEng(FEng).FRoot)
       and ((lmNoCopy in TDEng(FEng).GetFullLockMode(SelectedControls[LI]))
       or (lmNoDelete in TDEng(FEng).GetFullLockMode(SelectedControls[LI])))
       then
      begin
        Result := False;
        Break;
...
function TELDesignerSelectedControls.GetDefaultComponent: TComponent;
begin
  if Count > 0 then begin
    Result := Items[Count - 1];
    if Result is TDesignComponentEx then
      Result:=TDesignComponentEx(Result).FComponent;
  end else begin
    Result := nil;
  end;  
end;
...
begin
   ...
   for LI := 0 to Count - 1 do
    if Items[LI] <> TDEng(FDesigner.FEng).FRoot then
     if not (Items[LI] is TDesignComponentEx) then
       Items[LI].BringToFront;

   FDesigner.BringToFrontDesignComponent;
...
begin
  if Assigned(FOnDragDrop) then FOnDragDrop(Self, ASource, ATarget, AX, AY);
end;

procedure TELCustomDesigner.BringToFrontDesignComponent;
var
  i: Integer;
  List: TList;
begin
  ...
  List:=TList.Create;
  try
   for i:=0 to DesignControl.ControlCount-1 do
     if DesignControl.Controls[i] is TDesignComponentEx then
       List.Add(DesignControl.Controls[i]);
   for i:=0 to List.Count-1 do begin
     TControl(List.Items[i]).BringToFront;
     TDesignComponentEx(List.Items[i]).EditLabel.BringToFront;
   end;  
  finally
   List.Free;
  end;  
