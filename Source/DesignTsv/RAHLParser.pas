...
type
  ...
  TIParserStyle = (psNone, psPascal, psCpp, psPython, psVB, psHtml, psPerl, psCocoR);
      ...
      '*':
        ...
        begin
          ...
          if P[1]='/' then //IParserError(ieBadRemark, P - FpcProgram);
             Exit;
      '/':
        if (FStyle in [psPascal, psCpp, psCocoR]) and (P[1] = '/') then
        begin
          F := StrScan(P + 1, #13);
          if F = nil then F := StrEnd(P + 1);
          P := F;
        end
        else if (FStyle in [psCpp, psCocoR]) and (P[1] = '*') then
        begin
          F := P + 2;
          while true do
          begin
  ...
  begin
    ...
    begin
      ...
      SetString(Result, F, P - F);
            ...
            ((FStyle = psPerl) and (P[0] = '"') and ((P = FpcPos) or (P[-1] <> '/'))) then
