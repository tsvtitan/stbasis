...
begin
  ...
  for i := P downto 1 do
    // add by patofan
    if (S[i] in Separators ) {$IFDEF RA_D3H}and ( StrByteType( pchar(s) , i-1 ) <> mbTrailByte ){$ENDIF} then
...
begin
  ...
  if P > 1 then
    // add by patofan
    if (S[P] in Separators ) {$IFDEF RA_D3H}and ( StrByteType( pchar(s) , p-1 ) <> mbTrailByte ){$ENDIF} then
    // ending add by patofan
  ...
  while iBeg >= 1 do
    // add by patofan
    if ( S[iBeg] in Separators ) {$IFDEF RA_D3H}and ( StrByteType( pchar(s) , iBeg-1 ) <> mbTrailByte ){$ENDIF} then
    // ending add by patofan
      break
    else
      dec(iBeg);
  ...
  while iEnd <= Length(S) do
    if ( S[iEnd] in Separators ) {$IFDEF RA_D3H} and ( StrByteType( pchar(s) , iEnd-1 ) <> mbTrailByte ) {$ENDIF}
    then break else inc(iEnd);
  if iEnd > iBeg then
    Result := Copy(S, iBeg, iEnd - iBeg)
    else begin
      if not (S[P] in Separators) then begin
        Result := '';
      end else Result := S[P];
