...
const
  ...
  ValueMustBeBetween='Значение свойста должно быть между %d и %d';
...
procedure TELCustomPropsPage.ItemsChange;
  ...
  begin
    ...
    begin
      ...
      if TELPropsPageItem(AList[LI]).Expanded then _FillRows(TELObjectList(AList[LI]));
    end;
  end;
...
begin
  ...
  begin
    ...
    for LI := 0 to High(FRows) do
      if FRows[LI] <> nil then FRows[LI].FRow := -1;
    SetLength(FRows, 0);
    _FillRows(Items);
    ...
    begin
      ...
      begin
        if Row <> LActiveItem.FRow then Row := LActiveItem.FRow;
        Break;
      end;
...
begin
  ...
  begin
    ...
    if (LItem <> nil) and not LItem.IsOnExpandButton(Message.XPos) then
      if LItem.Expanded then LItem.Collapse else LItem.Expand;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Text = '' then PickList.ItemIndex := -1 else PickList.ItemIndex := PickList.Items.IndexOf(Text);
      { Calc PickList width }
...
begin
  case Key of
    ...
    VK_ESCAPE:  if TELCustomPropsPage(Grid).ActiveItem <> nil then
                begin
...
begin
  if (FEditor <> nil) and not TELCustomPropertyInspector(Owner).ReadOnly then FEditor.Edit;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      else if (praDialog in LPropAttrs) and not TELCustomPropertyInspector(Owner).ReadOnly then
...
begin
  ...
  if not (((pkProperties in PropKinds) and (LTypeInfo.Kind in tkProperties)) or
    ...
    (not (pkReadOnly in PropKinds) and not Assigned(APropInfo.SetProc) and (APropInfo.PropType^.Kind <> tkClass))
  then
  begin
    Result := nil;
    Exit;
  ...
  for LI := High(FEditors) downto 0 do
    if (LTypeInfo = FEditors[LI].TypeInfo) or
      ...
      GetTypeData(LTypeInfo).ClassType.InheritsFrom(GetTypeData(FEditors[LI].TypeInfo).ClassType))
    then
      if ((FEditors[LI].ObjectClass = nil) or (LClass.InheritsFrom(FEditors[LI].ObjectClass))) and
        ((FEditors[LI].PropName = '') or SameText(FEditors[LI].PropName, APropInfo.Name))
      then
        if (LBest = -1) or ((FEditors[LBest].ObjectClass = nil) and (FEditors[LI].ObjectClass <> nil)) or
          ...
          (GetTypeData(FEditors[LI].TypeInfo).ClassType.InheritsFrom(GetTypeData(FEditors[LBest].TypeInfo).ClassType)))
        then
        begin
          LBest := LI;
          Break;
        end;
  if LBest <> -1 then Result := FEditors[LBest].EditorClass else Result := nil;
  ...
  if Result = nil then
    case LTypeInfo.Kind of
      ...
      tkClass:          if (LTypeInfo = TypeInfo(TComponent)) or
...
begin
  if Assigned(OnGetComponent) then OnGetComponent(Self, AComponentName, AComponent);
end;
...
procedure TELCustomPropertyInspector.GetComponentName(AComponent: TComponent;
  var AName: string);
begin
  if Assigned(OnGetComponentName) then OnGetComponentName(Self, AComponent, AName);
end;
...
begin
  ...
  else if (AItem.Parent <> nil) and (TELPropertyInspectorItem(AItem.Parent).FEditor <> nil) and
...
begin
  if Assigned(OnGetCaptionColor) then OnGetCaptionColor(Self, APropTypeInfo, APropName, AColor);
end;
...
begin
  with FPropList^[AIndex] do Result := GetFloatProp(Instance, PropInfo);
end;
...
begin
  with FPropList^[AIndex] do Result := GetInt64Prop(Instance, PropInfo);
end;

function TELPropEditor.GetOrdValue(AIndex: Integer): Longint;
begin
  with FPropList^[AIndex] do Result := GetOrdProp(Instance, PropInfo);
end;
...
begin
  with FPropList^[AIndex] do Result := GetStrProp(Instance, PropInfo);
end;
...
begin
  with FPropList^[AIndex] do Result := GetVariantProp(Instance, PropInfo);
end;
...
begin
  for LI := 0 to FPropCount - 1 do
    with FPropList^[LI] do SetFloatProp(Instance, PropInfo, Value);
  Modified;
end;
...
begin
  for LI := 0 to FPropCount - 1 do
    with FPropList^[LI] do SetInt64Prop(Instance, PropInfo, Value);
  Modified;
end;
...
begin
  for LI := 0 to FPropCount - 1 do
    with FPropList^[LI] do SetOrdProp(Instance, PropInfo, Value);
  Modified;
end;
...
begin
  for LI := 0 to FPropCount - 1 do
    with FPropList^[LI] do SetStrProp(Instance, PropInfo, Value);
  Modified;
end;
...
begin
  for LI := 0 to FPropCount - 1 do
    with FPropList^[LI] do SetVariantProp(Instance, PropInfo, Value);
  Modified;
end;
  ...
  with GetTypeData(PropTypeInfo)^ do
    ...
    begin   // unsigned compare and reporting needed
      if (LI < Cardinal(MinValue)) or (LI > Cardinal(MaxValue)) then
        ...
        raise EELPropEditor.CreateFmt(ValueMustBeBetween,
          [Int64(Cardinal(MinValue)), Int64(Cardinal(MaxValue))]);
    end
    else if (LI < MinValue) or (LI > MaxValue) then
      raise EELPropEditor.CreateFmt(ValueMustBeBetween, [MinValue, MaxValue]);
  SetOrdValue(LI);
end;
...
var
  LI: Longint;
begin
  if Length(Value) = 0 then LI := 0 else
    if Length(Value) = 1 then LI := Ord(Value[1]) else
      if Value[1] = '#' then LI := StrToInt(Copy(Value, 2, Maxint)) else
        raise EELPropEditor.Create(InvalidPropertyValue);
  with GetTypeData(PropTypeInfo)^ do
...
var
  LI: Integer;
begin
  for LI := 0 to Screen.Imes.Count - 1 do AValues.Add(Screen.Imes[LI]);
end;
...
begin
  ...
  try
    ...
    if LFontDialog.Execute then SetOrdValue(Longint(LFontDialog.Font));
  finally
    LFontDialog.Free;
  end;
end;
...
var
  LI: Integer;
begin
  for LI := Low(ModalResults) to High(ModalResults) do AValues.Add(ModalResults[LI]);
end;
...
begin
  AValues.Add(srNone);
  for LI := 1 to High(ShortCuts) do AValues.Add(ShortCutToText(ShortCuts[LI]));
end;
...
begin
  ...
  begin
    ...
    if not (LComponent is GetTypeData(PropTypeInfo)^.ClassType) then
      raise EPropertyError.Create(InvalidPropertyValue);
  end;
