...
type
  ...
  TTVNavigatorEx = class (TCustomPanel)
  private
    ...
    FFlat: Boolean;
    FTreeView: TBaseVirtualDBTree;
    procedure BtnMouseDown (Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure ClickHandler(Sender: TObject);
    function GetHints: TStrings;
    procedure HintsChanged(Sender: TObject);
    function GetTreeView: TBaseVirtualDBTree;
    procedure SetTreeView(Value: TBaseVirtualDBTree);
  ...
  published
    ...
    property TabOrder;
    property TabStop;
    property TreeView: TBaseVirtualDBTree read GetTreeView write SetTreeView;
...
constructor TTVNavigatorEx.Create(AOwner: TComponent);
begin
...
destructor TTVNavigatorEx.Destroy;
begin
  FHints.Free;
  inherited Destroy;
end;

procedure TTVNavigatorEx.InitButtons;
var
  I: TTVNavigateBtn;
  Btn: TTVNavButton;
  X: Integer;
...
begin
  ...
  begin
    ...
    FmtStr(ResName, 'TVEXN_%s', [BtnTypeName[I]]);
...
procedure TTVNavigatorEx.InitHints;
var
  I: Integer;
  J: TTVNavigateBtn;
begin
...
procedure TTVNavigatorEx.HintsChanged(Sender: TObject);
begin
  InitHints;
end;

procedure TTVNavigatorEx.SetFlat(Value: Boolean);
var
  I: TTVNavigateBtn;
begin
  if FFlat <> Value then
...
procedure TTVNavigatorEx.SetHints(Value: TStrings);
begin
   FHints.Assign(Value);
end;

function TTVNavigatorEx.GetHints: TStrings;
begin
  Result := FHints;
end;

procedure TTVNavigatorEx.GetChildren(Proc: TGetChildProc; Root: TComponent);
begin
end;

procedure TTVNavigatorEx.Notification(AComponent: TComponent;
...
procedure TTVNavigatorEx.SetVisible(Value: TTVButtonSet);
var
  I: TTVNavigateBtn;
  W, H: Integer;
begin
...
procedure TTVNavigatorEx.CalcMinSize(var W, H: Integer);
var
  Count: Integer;
  I: TTVNavigateBtn;
begin
...
procedure TTVNavigatorEx.SetSize(var W: Integer; var H: Integer);
var
  Count: Integer;
  I: TTVNavigateBtn;
...
procedure TTVNavigatorEx.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
  W, H: Integer;
begin
  W := AWidth;
...
procedure TTVNavigatorEx.WMSize(var Message: TWMSize);
var
  W, H: Integer;
begin
  inherited;
...
procedure TTVNavigatorEx.WMWindowPosChanging(var Message: TWMWindowPosChanging);
begin
  inherited;
...
procedure TTVNavigatorEx.ClickHandler(Sender: TObject);
begin
  BtnClick (TTVNavButton (Sender).Index);
end;

procedure TTVNavigatorEx.BtnMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TTVNavigatorEx.SetEnableButtons;
var
  nd: PVirtualNode;
  i: TTVNavigateBtn;
begin
  nd:=FTreeView.FocusedNode;
  if (nd<>FTreeView.RootNode)and(nd<>nil) then begin
   Buttons[nbPrior].Enabled:=true;
   Buttons[nbFirst].Enabled:=true;
   Buttons[nbNext].Enabled:=true;
   Buttons[nbLast].Enabled:=true;

   if nd=FTreeView.GetFirst then begin
     Buttons[nbPrior].Enabled:=false;
     Buttons[nbFirst].Enabled:=false;
   end;
   if nd=FTreeView.GetLast(nil) then begin
...
type
  TBaseVirtualTreeEx=class(TBaseVirtualTree)
  public
   
  end;

procedure TTVNavigatorEx.BtnClick(Index: TTVNavigateBtn);
var
  nd: PVirtualNode;
begin
  if (FTreeView <> nil) then begin
    if not (csDesigning in ComponentState) and Assigned(FBeforeAction) then
      ...
      case Index of
        nbPrior: begin
         if FTReeView.FocusedNode<>nil then begin
           nd:=FTReeView.GetPrevious(FTReeView.FocusedNode);
           if nd<>nil then begin
//             TBaseVirtualTreeEx(FTReeView).UnselectNodes(FTReeView.GetFirst,FTReeView.GetLast(nil);
             FTReeView.FocusedNode:=nd;
             FTReeView.Selected[nd]:=true;
             FTReeView.ScrollIntoView(nd,true);
           end;
         end;
        end;
        nbNext: begin
         if FTReeView.FocusedNode<>nil then begin
           nd:=FTReeView.GetNext(FTReeView.FocusedNode);
           if nd<>nil then begin
//             FTReeView.UnselectNodes(FTReeView.GetFirst,FTReeView.GetLast(nil);
             FTReeView.FocusedNode:=nd;
             FTReeView.Selected[nd]:=true;
             FTReeView.ScrollIntoView(nd,true);
           end;
        ...
        nbFirst: begin
         if FTReeView.TotalCount>0 then begin
          nd:=FTReeView.GetFirst;
          if nd<>nil then begin
//            FTReeView.UnselectNodes(FTReeView.GetFirst,FTReeView.GetLast(nil);
            FTReeView.FocusedNode:=nd;
            FTReeView.Selected[nd]:=true;
...
procedure TTVNavigatorEx.WMSetFocus(var Message: TWMSetFocus);
begin
  Buttons[FocusedButton].Invalidate;
end;

procedure TTVNavigatorEx.WMKillFocus(var Message: TWMKillFocus);
begin
  Buttons[FocusedButton].Invalidate;
end;

procedure TTVNavigatorEx.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TTVNavigatorEx.WMGetDlgCode(var Message: TWMGetDlgCode);
begin
  Message.Result := DLGC_WANTARROWS;
end;

procedure TTVNavigatorEx.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
end;

procedure TTVNavigatorEx.SetTreeView(Value: TBaseVirtualDBTree);
begin
  FTreeView:=Value;
...
function TTVNavigatorEx.GetTreeView: TBaseVirtualDBTree;
begin
  Result := FTreeView;
end;
...
begin
  ...
  if (GetFocus = Parent.Handle) and
     (FIndex = TTVNavigatorEx (Parent).FocusedButton) then
  begin
