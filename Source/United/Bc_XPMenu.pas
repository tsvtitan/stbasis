...
begin
  ...
  begin
    ...
    begin
      if FMenu.IsRightToLeft then                          // 2001-10-23: ja, required so that menubar at right side works
        inc(ARect.Right,2);  //needed for RightToLeft
      DrawGradient(ACanvas, ARect, FMenu.IsRightToLeft);
      if FMenu.IsRightToLeft then                          // 2001-10-23: ja
        Dec(ARect.Right,2);
...
//------
  ...
  if not (csDesigning in ComponentState) then
  begin
    if (FFlatMenu) and (not FTopMenu) and Assigned(FForm) then
    begin
      hDcM := ACanvas.Handle;
...
//-----
//  ActivateMenuItem(FMenuItem);  // to check for new sub items   // 2001-10-23: ja
end;
...
procedure TXPMenu.ControlBarPaint(Sender: TObject; Control: TControl;
  Canvas: TCanvas; var ARect: TRect; var Options: TBandPaintOptions);
var
  DRect, CRect: TRect;    // Rectangle for the grabber
...
begin
  ...
  // First background
  Canvas.Brush.Color := clBtnFace;//FFIconBackColor;//clBtnFace;
  ...
  begin
    ...
    Top := ARect.Top + 5;
    Right := Left + 3;
    Bottom := ARect.Bottom - 5;
  end;

  Canvas.Brush.Color := FFSeparatorColor;//RGB(160,160,160);
