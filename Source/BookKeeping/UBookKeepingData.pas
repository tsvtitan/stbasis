...
}  hMenuJRMagazinePostings: THandle;
   hMenuRBooksSubkontoSubkonto: THandle;
  hMenuRpts: THandle;
  hMenuRptsBookKeering: THandle;
//  hMenuRptsCashOrders: THandle;
  ...
  function ChangeSymbol(Str: string;a,b:char): String;
const
...
//  ConstSectionShortCut='KassaKDMHotKeys';
  ...
  // Interface Names
  ConstsMenuBookKeeping='Бухгалтерский учет';
//  ConstsMenuRptKassa='Касса';
  ...
  NameJRMagazinePostings='Журнал проводок';
...
}  SQLRbkMagazinePostings='select td.NAME,m.*,P1.PA_GROUPID,P2.PA_GROUPID,u1.name as useraddname,u2.name as userchangename '+
                         'from '+tbMAGAZINEPOSTINGS+' m,'+tbPLANACCOUNTS+' P1,'+ tbPLANACCOUNTS+' P2,'+tbDOCUM +' D,'+
                          tbTypeDoc+' td,'+tbUsers+'  u1,'+tbUsers+'  u2'+
                         ' where MP_DEBIT=P1.PA_ID AND MP_CREDIT=P2.PA_ID AND MP_DOCUM_ID=D.DOCUM_ID AND '+
                         'D.TYPEDOC_ID=TD.TYPEDOC_ID AND MP_WHOADD=u1.user_id AND MP_WHOCHANGE=u2.user_id';
  SQLJrMagazinePostings='select td.NAME,m.*,P1.PA_GROUPID,P2.PA_GROUPID,u1.name as useraddname,u2.name as userchangename '+
                         'from '+tbMAGAZINEPOSTINGS+' m,'+tbPLANACCOUNTS+' P1,'+ tbPLANACCOUNTS+' P2,'+tbDOCUM +' D,'+
                          tbTypeDoc+' td,'+tbUsers+'  u1,'+tbUsers+'  u2'+
                         ' where MP_DEBIT=P1.PA_ID AND MP_CREDIT=P2.PA_ID AND MP_DOCUM_ID=D.DOCUM_ID AND '+
                         'D.TYPEDOC_ID=TD.TYPEDOC_ID AND MP_WHOADD=u1.user_id AND MP_WHOCHANGE=u2.user_id';
