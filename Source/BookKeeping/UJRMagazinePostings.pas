...
uses
  ...
  UJRMainGrid,UMainUnited;

type
  TfmJRMagazinePostings = class(TfmJRMainGrid)
    pnSQL: TPanel;
    bibAdd: TBitBtn;
    bibChange: TBitBtn;
    bibDel: TBitBtn;
...
var
  fmJRMagazinePostings: TfmJRMagazinePostings;

implementation

uses UBookKeepingCode, UBookKeepingDM, UBookKeepingData,UEditJRMagazinePostings;

{$R *.DFM}

procedure TfmJRMagazinePostings.FormResize(Sender: TObject);
begin
  inherited;
  edSearch.Width:=pnMain.Width-89-edSearch.Left;
  pnGrid.Height := pnMain.Height-120;
  pnGrid.Width := pnMain.Width-89;
  PHeader.Top := pnMain.Height-120;
  PData.Top := pnMain.Height-60;
  PHeader.Width := pnGrid.Width;
  PData.Width := pnGrid.Width;
end;

procedure TfmJRMagazinePostings.FormCreate(Sender: TObject);
var
 cl: TColumn;
begin
 inherited;
 try
  Caption:=NameJRMagazinePostings;
...
procedure TfmJRMagazinePostings.FormDestroy(Sender: TObject);
begin
  inherited;
  if FormState=[fsCreatedMDIChild] then
   fmJRMagazinePostings:=nil;
  FrameSub.DeInitData;
  FrameSub.Free;
end;

function TfmJrMagazinePostings.GetSql: string;
begin
  Result:= inherited GetSql;
  if Trim(Result)<>'' then exit;
  Result:=SQLJrMagazinePostings+GetFilterString+GetLastOrderStr;
end;

function TfmJRMagazinePostings.CheckPermission: Boolean;
var
  isPerm: Boolean;
begin
...
procedure TfmJrMagazinePostings.ActiveQuery(CheckPerm: Boolean);
var
 sqls: String;
begin
 try
...
procedure TfmJrMagazinePostings.GridTitleClickWithSort(Column: TColumn; TypeSort: TTypeColumnSort);
var
  fn: string;
  id: string;
begin
...
procedure TfmJrMagazinePostings.GridDblClick(Sender: TObject);
begin
  if not Mainqr.Active then exit;
...
procedure TfmJRMagazinePostings.LoadFromIni;
begin
 inherited;
 try
...
procedure TfmJRMagazinePostings.SaveToIni;
begin
 Inherited;
 try
...
procedure TfmJRMagazinePostings.bibRefreshClick(Sender: TObject);
begin
  ActiveQuery(true);
end;

function TfmJrMagazinePostings.GetFilterString: string;
var
  FilInSide: string;
  wherestr: string;
...
procedure TfmJRMagazinePostings.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmJRMagazinePostings.bibDelClick(Sender: TObject);
var
  but: Integer;
...
procedure TfmJRMagazinePostings.InitModalParams(hInterface: THandle; Param: PParamJournalInterface);
begin
  pnSQL.Visible:=false;
  inherited;
end;

procedure TfmJRMagazinePostings.MainqrAfterScroll(DataSet: TDataSet);
var
  qr: TIBQuery;
  sqls: string;
  Sub: TStrings;
...
procedure TfmJRMagazinePostings.bibAddClick(Sender: TObject);
var
  fm: TfmEditJRMagazinePostings;
begin
 try
  if not Mainqr.Active then exit;
  fm:=TfmEditJRMagazinePostings.Create(nil);
  try
...
procedure TfmJRMagazinePostings.bibChangeClick(Sender: TObject);
var
  fm: TfmEditJRMagazinePostings;
  qr: TIBQuery;
  sqls: string;
...
procedure TfmJRMagazinePostings.bibViewClick(Sender: TObject);
var
  fm: TfmEditJRMagazinePostings;
  IdRec: Integer;
  qr: TIBQuery;
...
procedure TfmJRMagazinePostings.bibFilterClick(Sender: TObject);
var
  fm: TfmEditJRMagazinePostings;
  LstAccount: TStrings;
  Account: string;
