unit UBookKeepingCode;
                                                     
interface

{$I stbasis.inc}

uses Windows, Forms, UBookKeepingData, UMainUnited, Classes, UBookKeepingDM, graphics, dialogs,
...
uses ActiveX, Menus{, URBCashBasis, URBCashAppend}, URBPlanAccounts,
     URBKindSubkonto{, URBCashOrders}, URBSubkontoSubkonto,dbtree,tsvDbGrid{,
     URptCashOrders},UJRMagazinePostings;
...
//  FreeAndNil(fmRBCashOrders);
  FreeAndNil(fmJRMagazinePostings);
...
//  if MenuHandle=hMenuRBooksCashOrders then ViewInterface(hInterfaceRbkCashOrders,@TPRBI);
  if MenuHandle=hMenuJrMagazinePostings then ViewInterface(hInterfaceJrMagazinePostings,@TPJI);
...
var
  isFreeMenuRBooksBookKeeping: Boolean;
...
begin
  ...
  hMenuRBooksBookKeeping:=CreateMenuLocal(hMenuRBooks,ConstsMenuBookKeeping,ConstsMenuBookKeeping,tcmAddFirst);
{  hMenuRBooksCashBasis:=MENU_INVALID_HANDLE;
  ...
  if isPermissionOnInterfaceView(hInterfaceRbkKindSubkonto) then
   hMenuRBooksKindSubkonto:=CreateMenuLocal(hMenuRBooksBookKeeping,'Виды субконто',NameRbkKindSubkonto,tcmAddLast,MENU_INVALID_HANDLE,-1,0,MenuClickProc);
  hMenuRBooksPlanAccounts:=MENU_INVALID_HANDLE;
  if isPermissionOnInterfaceView(hInterfaceRbkPlanAccounts) then
   hMenuRBooksPlanAccounts:=CreateMenuLocal(hMenuRBooksBookKeeping,'План счетов',NameRbkPlanAccounts,tcmAddLast,MENU_INVALID_HANDLE,-1,0,MenuClickProc);
{  hMenuRBooksCashOrders:=MENU_INVALID_HANDLE;
  ...
  if isPermissionOnInterfaceView(hInterfaceRbkSubkontoSubkonto) then
   hMenuRBooksSubkontoSubkonto:=CreateMenuLocal(hMenuRBooksBookKeeping,'Связи субконто',NameRbkSubkontoSubkonto,tcmAddLast,MENU_INVALID_HANDLE,-1,0,MenuClickProc);
...
{  hMenuRptsKassa:=CreateMenuLocal(hMenuRpts,ConstsMenuRptKassa,ConstsMenuRptKassa,tcmAddFirst);
  ...
  hMenuJrMagazinePostings:=MENU_INVALID_HANDLE;
  if isPermissionOnInterfaceView(hInterfaceJrMagazinePostings) then
   hMenuJrMagazinePostings:=CreateMenuLocal(hMenuJournals,'Журнал проводок',NameJrMagazinePostings,tcmAddLast,MENU_INVALID_HANDLE,0,0,MenuClickProc);
  ...
  isFreeMenuRBooksBookKeeping:=(hMenuRBooksBookKeeping<>MENU_INVALID_HANDLE)and
                           ...
                           // (hMenuRBooksCashOrders=MENU_INVALID_HANDLE)and
                            (hMenuRBooksSubkontoSubkonto=MENU_INVALID_HANDLE);
  if isFreeMenuRBooksBookKeeping then
    if _FreeMenu(hMenuRBooksBookKeeping) then hMenuRBooksBookKeeping:=MENU_INVALID_HANDLE;
...
{  isFreeMenuRptsKassa:=(hMenuRptsKassa<>MENU_INVALID_HANDLE)and
  ...
  isFreeMenuRBooks:=(hMenuRBooks<>MENU_INVALID_HANDLE)and
                    (hMenuRBooksBookKeeping=MENU_INVALID_HANDLE);
...
{  hInterfaceRbkCashOrders:=CreateLocalInterface(NameRbkCashOrders,NameRbkCashOrders);
  ...
  hInterfaceJrMagazinePostings:=CreateLocalInterface(NameJRMagazinePostings,NameJrMagazinePostings,ttiJournal);
  CreateLocalPermission(hInterfaceJRMagazinePostings,tbMagazinePostings,ttpSelect,false);
  CreateLocalPermission(hInterfaceJRMagazinePostings,tbKindSubkonto,ttpSelect,false);
  CreateLocalPermission(hInterfaceJRMagazinePostings,tbPlanAccounts,ttpSelect,false);
  CreateLocalPermission(hInterfaceJRMagazinePostings,tbDocum,ttpSelect,false);
  CreateLocalPermission(hInterfaceJRMagazinePostings,tbCurrency,ttpSelect,false);
  CreateLocalPermission(hInterfaceJRMagazinePostings,tbMagazinePostings,ttpUpdate,false,ttiaChange);
  CreateLocalPermission(hInterfaceJRMagazinePostings,tbMagazinePostings,ttpInsert,false,ttiaAdd);
  CreateLocalPermission(hInterfaceJRMagazinePostings,tbMagazinePostings,ttpDelete,false,ttiaDelete);
  CreateLocalPermission(hInterfaceJRMagazinePostings,tbMPSubkonto,ttpSelect,false);
...
function CreateAndViewMagazinePostings (InterfaceHandle: THandle; Param: PParamJournalInterface): Boolean;

   function CreateAndViewAsMDIChild: Boolean;
   begin
     result:=false;
     try
      if fmJrMagazinePostings=nil then
        fmJrMagazinePostings:=TfmJrMagazinePostings.Create(Application);
      fmJrMagazinePostings.InitMdiChildParams(InterfaceHandle,Param);
      Result:=true;
     except
      {$IFDEF DEBUG} on E: Exception do Assert(false,E.message);{$ENDIF}
     end;
   ...
   function CreateAndViewAsModal: Boolean;
   var
     fm: TfmJrMagazinePostings;
   begin
    Result:=false;
    try
     fm:=nil;
     try
       fm:=TfmJrMagazinePostings.Create(nil);
       fm.InitModalParams(InterfaceHandle,Param);
       if fm.ShowModal=mrOk then begin
         fm.ReturnModalParams(Param);
         Result:=true;
...
begin
  ...
  case Param.Visual.TypeView of
    tviMdiChild: Result:=CreateAndViewAsMDIChild;
    tvibvModal: Result:=CreateAndViewAsModal;
    tviOnlyData: Result:=QueryForParamJournalInterface(IBDB,SQLJrMagazinePostings,Param);
  end;
end;
...
}   if InterfaceHandle=hInterfaceJrMagazinePostings then begin
...
{    if InterfaceHandle=hInterfaceRbkCashOrders then begin
      ...
      isRemove:=not _isPermissionOnInterface(InterfaceHandle,ttiaView);
      if isRemove then FreeAndNil(fmJRMagazinePostings)
      else if fmJRMagazinePostings<>nil then begin
       fmJRMagazinePostings.SetInterfaceHandle(InterfaceHandle);
       fmJRMagazinePostings.ActiveQuery(true);
      end;
    end;
     if InterfaceHandle=hInterfaceRbkSubkontoSubkonto then begin
      isRemove:=not _isPermissionOnInterface(InterfaceHandle,ttiaView);
