...
type
  ...
  private
    Count,Button,IdAc,IdRec: integer;
    isForPA: Boolean;
  ...
  public
    ...
    ChangeFlag: Boolean;
    TableWithSubkonto,FieldIdRecord,FieldIdSubkonto,FieldIdInSubkonto: string;
    function InitData(idAccount,Id:Integer;Table,FId,FIdSub,FIdInSub: string; isPA:Boolean=false): Boolean;
    function CheckFieldsFill :Boolean;
    function GetIdSub: Boolean;
    function GetIdInSub(): Boolean;
...
uses UBookKeepingData, UMainUnited, UBookKeepingCode;

{$R *.DFM}

function TFrameSubkonto.InitData(idAccount,Id:Integer;Table,FId,FIdSub,FIdInSub: string; isPA:Boolean=false): Boolean;
var
 qr: TIBQuery;
 sqls: string;
 i,k: integer;
begin
 ...
 try
  ...
  isForPA := isPA;
  IdRec := Id;
  TableWithSubkonto := Table;
  FieldIdRecord:=FId;
  FieldIdSubkonto:=FIdSub;
  FieldIdInSubkonto:=FIdInSub;
...
function TFrameSubkonto.GetIdInSub(): Boolean;
var
  qr: TIBQuery;
  sqls: string;
  i: integer;
begin
 ...
 try
  ...
  qr.Transaction := IBT;
  try
    sqls := 'select pk.PAKS_LEVEL,ks.subkonto_name,ks.subkonto_tablename,ks.subkonto_fieldwithid,'+
            'ks.subkonto_fieldwithtext,ks.subkonto_nameinterface,ts.'+FieldIdRecord+',ts.'+
             FieldIdSubkonto+',ts.'+FieldIdInSubkonto+' from '+tbPlanAccounts_KindSubkonto+' pk,'+
             tbKindSubkonto+' ks, '+TableWithSubkonto+' ts '+
            ' where ts.'+FieldIdRecord+'='+IntToStr(IdRec)+' and ts.'+FieldIdSubkonto+'=ks.subkonto_id and '+
            ' pk.PAKS_PA_ID='+IntToStr(IdAc)+'and pk.PAKS_SUBKONTO_ID=ts.'+FieldIdSubkonto+
            ' and ts.'+FieldIdInSubkonto+' is not null order by PAKS_LEVEL asc';
    ...
    if not qr.IsEmpty then begin
      ...
      while not qr.Eof do begin
        idinSub[i]:=qr.FieldByName(FieldIdInSubkonto).AsInteger;
        qr.Next;
        i:=i+1;
      end;
    end;
    i:=1;
   while idinSub[i]<>0 do begin
     if (i>TableNameSubkonto.Count) then exit;
     sqls:='select * from '+TableNameSubkonto[i-1]+
           ' where '+FieldId[i-1]+'='+IntToStr(idinSub[i]);
     qr.SQL.Clear;
     qr.SQL.Add(sqls);
     qr.Open;
     if not qr.IsEmpty then begin
       DataSubkonto.Add(Trim(qr.FieldByName(FieldText[i-1]).AsString));
       TEdit(FindComponent('ESub'+IntToStr(i))).Text := Trim(qr.FieldByName(FieldText[i-1]).AsString);
     end;
     i:=i+1;
   end;
  finally
...
begin
 ...
 try
  ...
  try
    ...
    if not isForPA then
      Count := c;
    sqls := 'select pk.PAKS_SUBKONTO_ID,pk.PAKS_LEVEL,ks.subkonto_name,ks.subkonto_tablename,ks.subkonto_fieldwithid,'+
            'ks.subkonto_fieldwithtext,ks.subkonto_nameinterface from '+tbPlanAccounts_KindSubkonto+' pk,'+
             tbKindSubkonto+' ks '+
            ' where pk.PAKS_SUBKONTO_ID=ks.SUBKONTO_ID and pk.PAKS_PA_ID='+IntToStr(IdAc)+
            ' order by PAKS_LEVEL asc';
{    sqls := 'select pk.PAKS_LEVEL,ks.subkonto_name,ks.subkonto_tablename,ks.subkonto_fielwithid,'+
            'ks.subkonto_fieldwithtext,ks.subkonto_nameinterface,ts.'+FieldIdRecord+',ts.'+
             FieldWithIdSubkonto+',ts.'+FieldWithIdInSubkonto+' from '+tbPlanAccounts_KindSubkonto+' pk,'+
             tbKindSubkonto+' ks, '+TableWithSubkonto+' ts '+
            ' where ts.'+FieldIdRecord+'='+IntToStr(IdRec)+' and ts.'+FieldIdSubkonto+'=ks.subkonto_id and'+
            'pk.PAKS_SUBKONTO_ID=ks.SUBKONTO_ID and pk.PAKS_PA_ID='+IntToStr(IdAc)+
    ...
    if c<>0 then begin
      ...
      for i:=1 to c do begin
        ...
        idLevelSub[k]:=qr.FieldByName('PAKS_LEVEL').AsInteger;
