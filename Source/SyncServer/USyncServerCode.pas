...
begin
  ...
  hMenuServiceSync:=CreateMenuLocal(hMenuService,'Синхронизация','Справочники синхронизации',tcmAddFirst);
  hMenuRBooksSync_Office:=MENU_INVALID_HANDLE;
  if isPermissionOnInterfaceView(hInterfaceRbkSync_Office) then
    hMenuRBooksSync_Office:=CreateMenuLocal(hMenuServiceSync,'Офисы',NameRbkSync_Office,tcmAddLast,MENU_INVALID_HANDLE,0,0,MenuClickProc);

  hMenuRBooksSync_Package:=MENU_INVALID_HANDLE;
  if isPermissionOnInterfaceView(hInterfaceRbkSync_Package) then
    hMenuRBooksSync_Package:=CreateMenuLocal(hMenuServiceSync,'Пакеты',NameRbkSync_Package,tcmAddLast,MENU_INVALID_HANDLE,0,0,MenuClickProc);

  hMenuRBooksSync_Object:=MENU_INVALID_HANDLE;
  if isPermissionOnInterfaceView(hInterfaceRbkSync_Object) then
    hMenuRBooksSync_Object:=CreateMenuLocal(hMenuServiceSync,'Объекты',NameRbkSync_Object,tcmAddLast,MENU_INVALID_HANDLE,0,0,MenuClickProc);

  hMenuRBooksSync_OfficePackage:=MENU_INVALID_HANDLE;
  if isPermissionOnInterfaceView(hInterfaceRbkSync_OfficePackage) then
    hMenuRBooksSync_OfficePackage:=CreateMenuLocal(hMenuServiceSync,'Офисные пакеты',NameRbkSync_OfficePackage,tcmAddLast,MENU_INVALID_HANDLE,0,0,MenuClickProc);
