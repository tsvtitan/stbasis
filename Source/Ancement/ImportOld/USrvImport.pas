...
uses
  ...
  RxMemDS, IBDatabase, IBCustomDataSet, IBQuery, comctrls;
      ...
      try
       ...
       for i:=EndValue downto StartValue do begin
        ...
        if ListParent<>nil then begin
         ...
         P.treeheading_id:=GetValueByParamRBookInterface(@TPRBI,i,'treeheading_id',varInteger);
...
type
  ...
  function GetStringSkipSetOfChar(s: string; SetOfChar: TSetOfChar): string;
  var
    i: Integer;
    tmps: string;
  begin
     ...
     for i:=0 to TCPB.Max-1 do begin
      ...
      with fmParent do begin
        ...
        if Trim(s)<>'' then begin
         ...
         if not isChangeTree then begin
       ...
       try
        ...
        with fmParent do begin
         ...
         sqls:='Insert into '+tbAnnouncement+
               ...
               ','+iff(MemTable.FieldByName('textannouncement').Value=null,'null',QuotedStr(MemTable.FieldByName('textannouncement').AsString))+
  ...
  if od.Execute then begin
    try
     ...
     try
      ...
      ViewCount;
      Update;
      reLoad.Lines.LoadFromFile(od.FileName);
     finally
...
procedure TfmSrvImport.OnRptLoadTerminate(Sender: TObject);
begin
  FreeAndNil(RptLoad);
  bibBreakClick(nil);
end;
...
begin
  ...
  if _ViewInterfaceFromName(NameRbkRelease,@TPRBI) then begin
   if ifExistsDataInParamRBookInterface(@TPRBI) then begin
    ...
    edRelease.Text:=NumRelease+' от '+DateToStr(GetFirstValueFromParamRBookInterface(@TPRBI,'daterelease'));
   end;
  end;
end;
   ...
   case Key of
    ...
    VK_F4: begin
      if not MemTable.isEmpty then begin
       but:=DeleteWarning('текущее объявление ?');
