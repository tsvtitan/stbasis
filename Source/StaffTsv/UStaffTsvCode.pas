...
uses ActiveX,Menus,tsvDbGrid,dbtree,
     ...
     URBCurrency, URBRateCurrency, URBTypeEncouragements,URBTypeResQual,
...
function RefreshPlant(InterfaceHandle: THandle; Param: PParamRefreshRBookInterface): Boolean;
begin
  Result:=false;
  if fmRBPlant=nil then exit;
  fmRBPlant.ActiveQuery(false);
  Result:=fmRBPlant.LocateByRefreshParam(Param);
end;
...
begin
  Result:=false;
  if InterfaceHandle=hInterfaceRbkPlant then begin
    if not isValidPointer(Param,SizeOf(TParamRefreshRBookInterface)) then exit;
    Result:=RefreshPlant(InterfaceHandle,Param);
  end;
end;
