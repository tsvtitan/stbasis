unit UEditRBLinkTypeDocNumerator;
...
type
  TfmEditRBLinkTypeDocNumerator = class(TfmEditRB)
    edTypeDoc: TEdit;
    bibTypeDoc: TBitBtn;
    lbTypeDoc: TLabel;
...
var
  fmEditRBLinkTypeDocNumerator: TfmEditRBLinkTypeDocNumerator;
...
procedure TfmEditRBLinkTypeDocNumerator.FilterClick(Sender: TObject);
begin
  ModalResult:=mrOk;
end;

function TfmEditRBLinkTypeDocNumerator.AddToRBooks: Boolean;
var
  qr: TIBQuery;
  sqls: string;
  id: string;
begin
 ...
 try
  ...
  try
    ...
    sqls:='Insert into '+tbLinkTypeDocNumerator+
...
procedure TfmEditRBLinkTypeDocNumerator.AddClick(Sender: TObject);
begin
...
function TfmEditRBLinkTypeDocNumerator.UpdateRBooks: Boolean;
var
  qr: TIBQuery;
  sqls: string;
  id: String;
...
//    OldTypeNumerator_id:=TypeNumerator_id;
  //    id:=inttostr(oldStorePlace_id);
    ...
    qr.Transaction:=IBTran;
    qr.Transaction.Active:=true;
    sqls:='Update '+tbLinkTypeDocNumerator+
    ...
    begin
      sqls:='Delete from '+tbLinkTypeDocNumerator+' where TypeDoc_id='+inttostr(OldTypeDoc_id) +
...
procedure TfmEditRBLinkTypeDocNumerator.ChangeClick(Sender: TObject);
begin
...
function TfmEditRBLinkTypeDocNumerator.CheckFieldsFill: Boolean;
begin
  Result:=false;
...
procedure TfmEditRBLinkTypeDocNumerator.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfmEditRBLinkTypeDocNumerator.chbInFirstClick(Sender: TObject);
begin
  ChangeFlag:=true;
end;

procedure TfmEditRBLinkTypeDocNumerator.edNameStorePlaceChange(Sender: TObject);
begin
  ChangeFlag:=true;
end;

procedure TfmEditRBLinkTypeDocNumerator.bibTypeDocClick(Sender: TObject);
var
...
procedure TfmEditRBLinkTypeDocNumerator.edTypeDocKeyDown(Sender: TObject;
...
procedure TfmEditRBLinkTypeDocNumerator.bibTypeNumeratorClick(Sender: TObject);
var
  TPRBI: TParamRBookInterface;
  n:Ansistring;
begin
...
procedure TfmEditRBLinkTypeDocNumerator.edTypeNumeratorKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if (Key=VK_DELETE)or(Key=VK_BACK) then begin
