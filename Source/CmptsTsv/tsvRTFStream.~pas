...
type
  TtsvRTFStream=class(TMemoryStream)
  private
    FClosed: Boolean;
...
constructor TtsvRTFStream.Create;
begin
  FClosed:=false;
  FPosColorTable:=0;
...
destructor TtsvRTFStream.Destroy;
begin
  Clear;
  FFontTable.Free;
  FColorTable.Free;
...
procedure TtsvRTFStream.WriteString(S: string);
begin
  Write(Pointer(S)^,Length(S));
end;

procedure TtsvRTFStream.InsertString(Pos: Integer; S: string);
var
...
procedure TtsvRTFStream.CreateHeader;
begin
  WriteString('\rtf');
  WriteString('\ansi');
...
procedure TtsvRTFStream.OpenBody;
begin
...
procedure TtsvRTFStream.CloseBody;
begin
  WriteString('\par');
end;

procedure TtsvRTFStream.Open;
begin
  FClosed:=false;
  WriteString('{');
  CreateHeader;
...
function TtsvRTFStream.GetFontIdentFromFontTable(Font: TFont): string;
...
function TtsvRTFStream.GetColorIdentFromColorTable(Font: TFont): string;
var
  r,g,b: Byte;
  val: Integer;
  S: string;
...
procedure TtsvRTFStream.CreateString(S: String; Font: TFont; NewLine: Boolean=false);
var
  i: Integer;
begin
