...
{ TXMLDocument }
  ...
  TXMLDocument = class(TComponent, IXMLDocument, IXMLDocumentAccess)
  private
    FXMLData: DOMString;
    FSrcStream: TStream;
  ...
  public
    ...
    procedure LoadFromDOMXML(const XML: DOMString); overload;
    procedure Refresh;
...
function LoadDOMXMLData(const XMLData: DOMString): IXMLDocument; overload;
...
function Utf8ToUnicodeEx(Dest: PWideChar; MaxDestChars: Cardinal; Source: PChar; SourceBytes: Cardinal): Cardinal;
var
  i, count: Cardinal;
  c: Byte;
  wc: Cardinal;
...
begin
  ...
  if Source <> nil then
    while Source[len] <> #0 do
      Inc(len);
  Result := Utf8ToUnicodeEx(Dest, MaxChars, Source, len);
end;
...
begin
  ...
  L := Utf8ToUnicodeEx(PWideChar(Temp), Length(Temp)+1, PChar(S), Length(S));
  if L > 0 then
    SetLength(Temp, L-1)
  else
    Temp := '';
...
function UnicodeToUtf8Ex(Dest: PChar; MaxDestBytes: Cardinal; Source: PWideChar; SourceChars: Cardinal): Cardinal;
var
  i, count: Cardinal;
  c: Cardinal;
begin
...
begin
  ...
  L := UnicodeToUtf8Ex(PChar(Temp), Length(Temp)+1, PWideChar(WS), Length(WS));
  if L > 0 then
    SetLength(Temp, L-1)
  else
    Temp := '';
...
begin
  Result := SupportsEx(Node, ItemInterface)
...
procedure TXMLDocument.LoadFromDOMXML(const XML: DOMString);
begin
  SetActive(False);
  SetXMLStrings('');
  FXMLData := XML;
