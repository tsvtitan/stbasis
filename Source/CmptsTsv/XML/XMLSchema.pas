...
uses SysUtils, Classes, xmldom, XMLDoc, XMLIntf;
...
{ IXMLSchemaTranslator }
  ...
  IXMLSchemaTranslator = interface
...
{ IXMLSchemaTranslatorFactory }
  ...
  IXMLSchemaTranslatorFactory = interface
...
function SupportsEx(const Instance: IUnknown; const IID: TGUID): Boolean;
...
begin
  ...
  begin
    if SupportsEx(ContentNode, IXMLSimpleTypeRestriction) then
      Result := sdmRestriction
    else if SupportsEx(ContentNode, IXMLSimpleTypeList) then
      Result := sdmList
    else if SupportsEx(ContentNode, IXMLSimpleTypeUnion) then
      Result := sdmUnion;
  end;
end;
...
begin
  if not SupportsEx(DocumentElement, IXMLSchemaDef) then
