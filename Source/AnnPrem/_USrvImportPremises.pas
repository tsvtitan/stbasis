...
procedure TfmSrvImportPremises.TranslateData(FileName: String);
  ...
  function GetOutFormat: Variant;
  begin
    Result:=GetFirstValueBySQL(IBDB,Format('SELECT FORMAT FROM AP_OPERATION WHERE AP_OPERATION_ID=%d',[edOperation.Tag]),'FORMAT');
  end;
...
begin
  if FDataPreview.Active and not FDataPreview.IsEmpty then begin
    OutFormat:=Trim(VarToStr(GetOutFormat));
    if Trim(OutFormat)='' then
      ...
      while not FDataPreview.Eof do begin
        ...
        try
          ...
          case edOperation.Tag of
            ...
            23, // Коммерческая/Куплю
            24  // Коммерческая/Продам
            : begin
              L2:='';
              L2:=GetFirstValueBySQL(IBDB,Format('SELECT LINK FROM AP_OPERATION WHERE AP_OPERATION_ID=%d',[edOperation.Tag]),'LINK');
              Str.Add(L2);
            ...
            26, // Загородняя/Продам
            43, // Загородняя/Сдам
            46, // Загородняя/Сниму
            44 // Загороднее/Продам долевое
            : begin
              L2:='';
              L2:=GetFirstValueBySQL(IBDB,Format('SELECT LINK FROM AP_OPERATION WHERE AP_OPERATION_ID=%d',[edOperation.Tag]),'LINK');
              Str.Add(L2);
