...
begin
  if (hMenuService<>MENU_INVALID_HANDLE)or
   ...
   tbar1:=CreateToolBarLocal('Синхронизация','Панель синхронизации',0);
   ListToolBarHandles.Add(Pointer(tbar1));
   if hMenuSvcSync<>MENU_INVALID_HANDLE then
     hToolButtonSync:=CreateToolButtonLocal(tbar1,'Синхронизировать',NameSvcSync,0,ToolButtonClickProc);
...
begin
  ...
  if isPermissionOnInterfaceView(hInterfaceSvcSync) then
    hMenuSvcSync:=CreateMenuLocal(hMenuServiceSync,'Синхронизировать',NameSvcSync,tcmAddLast,MENU_INVALID_HANDLE,0,0,MenuClickProc);

  hMenuRBooksSync_Connection:=MENU_INVALID_HANDLE;
  if isPermissionOnInterfaceView(hInterfaceRbkSync_Connection) then
    hMenuRBooksSync_Connection:=CreateMenuLocal(hMenuServiceSync,'Соединения',NameRbkSync_Connection,tcmAddLast,MENU_INVALID_HANDLE,-1,0,MenuClickProc);
