...
uses UMainUnited, UInvalidTsvCode, UInvalidTsvDM, UInvalidTsvData, UEditRBBranch;
...
begin
  ...
  Result:=SQLRbkBranch+GetFilterString+GetLastOrderStr;
end;
...
begin
 try
   ...
   fn:=Column.FieldName;
   id:=MainQr.fieldByName('branch_id').asString;
   SetLastOrderFromTypeSort(fn,TypeSort);
   ActiveQuery(false);
   MainQr.First;
   MainQr.Locate('branch_id',id,[loCaseInsensitive]);
 except
  {$IFDEF DEBUG} on E: Exception do Assert(false,E.message); {$ENDIF}
 end;
end;
...
begin
  if not Mainqr.Active then exit;
  fm:=TfmEditRBBranch.Create(nil);
  try
    fm.TypeEditRBook:=terbAdd;
    fm.ChangeFlag:=false;
    if fm.ShowModal=mrok then begin
     ActiveQuery(false);
     MainQr.Locate('branch_id',fm.oldbranch_id,[loCaseInsensitive]);
    end;
  finally
    fm.Free;
  end;
...
begin
  ...
  try
    ...
    fm.oldBranch_id:=MainQr.FieldByName('branch_id').AsInteger;
    fm.ChangeFlag:=false;
    if fm.ShowModal=mrok then begin
     ActiveQuery(false);
     MainQr.Locate('branch_id',fm.oldBranch_id,[loCaseInsensitive]);
  ...
  begin
   ...
   try
    ...
    try
     ...
     sqls:='Delete from '+tbBranch+' where branch_id='+
          Mainqr.FieldByName('branch_id').asString;
     qr.sql.Add(sqls);
     qr.ExecSQL;
...
begin
  if Mainqr.RecordCount=0 then exit;
  but:=DeleteWarningEx('отделение <'+Mainqr.FieldByName('name').AsString+'> ?');
