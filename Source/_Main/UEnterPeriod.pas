...
uses
  ...
  StdCtrls, ComCtrls, Buttons, ExtCtrls, Spin, UMainUnited;
...
begin
 try
  ...
  tmps:=inttostr(CurKvartal)+' '+kvstr+' '+inttostr(CurKvartalYear)+' г.';
  edKvartal.Text:=tmps;
...
begin
  if not isChangeKvartal then begin
    isChangeKvartal:=true;
    edKvartal.Text:=inttostr(CurKvartal)+' '+kvstr+' '+inttostr(CurKvartalYear)+' г.';
  end;
  edYear.SelectAll;
end;
...
begin
  ...
  tmps:=monthstr+' '+inttostr(CurMonthYear)+' г.';
  edMonth.Text:=tmps;
  if self.Visible then
   edMonth.SetFocus;
  edMonth.SelectAll;
...
begin
  if not isChangeMonth then begin
    isChangeMonth:=true;
    monthstr:=LongMonthNames[CurMonth];
    edMonth.Text:=monthstr+' '+inttostr(CurMonthYear)+' г.';
  end;
end;

procedure TfmEnterPeriod.LoadPeriod(P: PInfoEnterPeriod);
begin
  P.TypePeriod:=TTypeEnterPeriod(ReadParam(ClassName,'TypePeriod',Integer(P.TypePeriod)));
  P.DateBegin:=ReadParam(ClassName,'DateBegin',P.DateBegin);
  P.DateEnd:=ReadParam(ClassName,'DateEnd',P.DateEnd);
end;
...
begin
  ...
  try
    ...
    GetPeriod(P);
    WriteParam(ClassName,'TypePeriod',Integer(P.TypePeriod));
    WriteParam(ClassName,'DateBegin',P.DateBegin);
    WriteParam(ClassName,'DateEnd',P.DateEnd);
  finally
   dispose(P);
  end;
end;
