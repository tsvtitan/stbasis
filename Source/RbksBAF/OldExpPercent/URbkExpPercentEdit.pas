...
type
  TFmRbkExpPercentEdit = class(TFmRbkEdit)
    LbTypePay: TLabel;
    EdTypePay: TEdit;
    BtCallTypePay: TButton;
    LbExperience: TLabel;
    LbPercent: TLabel;
    EdExperience: TMaskEdit;
    EdPercent: TMaskEdit;
    procedure BtCallTypePayClick(Sender: TObject);
...
begin
  Result:=true;
  If trim(EdTypePay.text) ='' then
  begin
    ShowError(Handle,Format(ConstFieldNoEmpty,[lbTypePay.Caption]));
    btCallTypePay.SetFocus;
    Result:=false;
    exit;
  end;
...
procedure TFmRbkExpPercentEdit.BtCallTypePayClick(Sender: TObject);
var
  P: PTypePayParams;
begin
 try
  getMem(P,sizeof(TTypePayParams));
  try
   ZeroMemory(P,sizeof(TTypePayParams));
   P.TypePay_id:=TypePay_id;
   if _ViewEntryFromMain(tte_rbksTypePay,p,true) then
   begin
     ChangeFlag:=true;
     TypePay_id:=P.TypePay_id;
     EdTypePay.Text:=P.Name;
   end;
  finally
    FreeMem(P,sizeof(TTypeDocParams));
  end;
 except
   {$IFDEF DEBUG} on E: Exception do Assert(false,E.message); {$ENDIF}
 end;
