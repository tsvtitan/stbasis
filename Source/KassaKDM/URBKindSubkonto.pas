...
//    procedure Button1Click(Sender: TObject);
  private
    ...
    isFindSubkonto_FieldWithText,isFindSubkonto_NameInterface,isFindSubkonto_Level: Boolean;
    FindSubkonto_Name,FindSubkonto_TableName,FindSubkonto_FieldWithId,
    FindSubkonto_FieldWithText,FindSubkonto_NameInterface,FindSubkonto_Level: String;
  protected
...
uses UMainUnited, UKassaKDMCode, UKassaKDMDM, UKassaKDMData, UEditRBKindSubkonto;
...
begin
 ...
 try
  ...
  cl:=Grid.Columns.Add;
  cl.FieldName:='Subkonto_level';
  cl.Title.Caption:='Уровень';
  cl.Width:=180;
 ...
 try
  ...
  try
   ...
   SetImageFilter(isFindSubkonto_Name or isFindSubkonto_TableName or isFindSubkonto_FieldWithId
                    or isFindSubkonto_FieldWithText or isFindSubkonto_NameInterface or isFindSubkonto_Level);
   ViewCount;
  finally
...
begin
 ...
 try
   ...
   FindSubkonto_Level:=ReadParam(ClassName,'Subkonto_Level',FindSubkonto_Level);
...
begin
 ...
 try
   ...
   WriteParam(ClassName,'Subkonto_Level',FindSubkonto_Level);
...
begin
 try
  ...
  try
    ...
    fm.ELevel.Text:=Mainqr.fieldByName('subkonto_level').AsString;
    fm.udLevel.Position:=Mainqr.fieldByName('subkonto_level').AsInteger;
...
//  fm.Edit.ReadOnly:=false;
  ...
  if Trim(FindSubkonto_Level)<>'' then begin
    fm.ELevel.Text:=FindSubkonto_Level;
    fm.udLevel.Position := StrToInt(FindSubkonto_Level);
  end;

  fm.cbInString.Visible:=true;
  fm.bibClear.Visible:=true;
  fm.cbInString.Checked:=FilterInSide;
  fm.ELevel.ReadOnly := false;
  ...
  if fm.ShowModal=mrOk then begin
    ...
    FindSubkonto_Level := Trim(fm.ELevel.Text);
...
begin
    ...
    isFindSubkonto_Level:=Trim(FindSubkonto_Level)<>'';

    if isFindSubkonto_Name or isFindSubkonto_TableName or isFindSubkonto_FieldWithId or
       isFindSubkonto_FieldWithText or isFindSubkonto_NameInterface or isFindSubkonto_Level then begin
     wherestr:=' where ';
    end else begin
    end;
    ...
    if FilterInside then FilInSide:='%';
     ...
     if isFindSubkonto_Level then begin
        addstr6:=' subkonto_level='+FindSubkonto_Level+' ';
     end;
     ...
     if isFindSubkonto_NameInterface and (isFindSubkonto_FieldWithId or isFindSubkonto_Name or isFindSubkonto_TableName
       ...
       and4 := ' AND ';
     if isFindSubkonto_Level and (isFindSubkonto_FieldWithId or isFindSubkonto_Name or isFindSubkonto_TableName
        or isFindSubkonto_FieldWithText or isFindSubkonto_NameInterface) then
       and5 := ' AND ';
     Result:=wherestr+addstr1+and1+addstr2+and2+addstr3+and3+addstr4+and4+addstr5+and5+addstr6;
end;
