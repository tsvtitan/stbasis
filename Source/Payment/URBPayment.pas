...
begin
 ...
 try
  ...
  cl.Title.Caption:='Карта оплаты';
  cl.Width:=150;

  cl:=Grid.Columns.Add;
  cl.FieldName:='purpose_name';
  cl.Title.Caption:='Назначение';
  cl.Width:=150;

  cl:=Grid.Columns.Add;
  cl.FieldName:='how_much';
  cl.Title.Caption:='Сумма платежа';
  cl.Width:=50;
...
begin
  ...
  try
    ...
    if _ViewInterfaceFromName(NameRbkCard,@TPRBI) then begin
      ...
      fm.lbLimit.Caption:=Format('Остаток: %d',[card_nominal-fm.GetLimitByCard]);
    end;
...
begin
  if Mainqr.RecordCount=0 then exit;
  but:=DeleteWarningEx('оплату по карте <'+Mainqr.FieldByName('num_card').AsString+
                       '> за '+'<'+Mainqr.FieldByName('date_payment').AsString+'>'+'?');
