unit UEditRBPms_Regions_Correspond;
...
type
  TfmEditRBPms_Regions_Correspond = class(TfmEditRB)
    lbCityRegion: TLabel;
    edCityRegion: TEdit;
    edRegion: TEdit;
    lbRegion: TLabel;
...
var
  fmEditRBPms_Regions_Correspond: TfmEditRBPms_Regions_Correspond;

implementation

uses UPremisesTsvCode, UPremisesTsvData, UMainUnited, URBPms_Regions_Correspond;

{$R *.DFM}

procedure TfmEditRBPms_Regions_Correspond.FilterClick(Sender: TObject);
begin
  ModalResult:=mrOk;
end;

function TfmEditRBPms_Regions_Correspond.AddToRBooks: Boolean;
var
  qr: TIBQuery;
  sqls: string;
...
begin
 ...
 try
  ...
  try
    ...
    sqls:='Insert into '+tbPms_Regions_Correspond+
    ...
    TfmRBPms_Regions_Correspond(fmParent).IBUpd.InsertSQL.Clear;
    TfmRBPms_Regions_Correspond(fmParent).IBUpd.InsertSQL.Add(sqls);

    with TfmRBPms_Regions_Correspond(fmParent).MainQr do begin
      Insert;
    ...
    TfmRBPms_Regions_Correspond(fmParent).ActiveQuery(true);
    Result:=true;
  finally
    qr.Free;
...
procedure TfmEditRBPms_Regions_Correspond.AddClick(Sender: TObject);
begin
...
function TfmEditRBPms_Regions_Correspond.UpdateRBooks: Boolean;
var
  qr: TIBQuery;
  sqls: string;
...
begin
 ...
 try
  ...
  try
    ...
    sqls:='Update '+tbPms_Regions_Correspond+
    ...
    TfmRBPms_Regions_Correspond(fmParent).IBUpd.ModifySQL.Clear;
    TfmRBPms_Regions_Correspond(fmParent).IBUpd.ModifySQL.Add(sqls);
...
procedure TfmEditRBPms_Regions_Correspond.ChangeClick(Sender: TObject);
begin
...
function TfmEditRBPms_Regions_Correspond.CheckFieldsFill: Boolean;
begin
  Result:=true;
...
procedure TfmEditRBPms_Regions_Correspond.edCityRegionChange(Sender: TObject);
begin
  inherited;
  ChangeFlag:=true;
end;

procedure TfmEditRBPms_Regions_Correspond.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfmEditRBPms_Regions_Correspond.btCityRegionClick(Sender: TObject);
 var
...
procedure TfmEditRBPms_Regions_Correspond.btRegionClick(Sender: TObject);
 var
  TPRBI: TParamRBookInterface;
begin
  FillChar(TPRBI,SizeOf(TPRBI),0);
