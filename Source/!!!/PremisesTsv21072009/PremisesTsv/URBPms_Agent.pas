...
type
  ...
  private
    IndexFindName, IndexFindNote, IndexFindOffice, IndexFindPhone: Integer;
  protected
...
begin
 ...
 // IndexFindPhone:=Filters.Add('a.phone',tdbcLike,false).Index;
...
begin
 try
   ...
   if UpperCase(fn)=UpperCase('phone') then fn:='a.phone';
...
begin
 ...
 try
  ...
  fm.lbPhone.Enabled:=false;
  fm.edPhone.Enabled:=false;
  fm.edPhone.Color:=clBtnFace;
  ...
  //fm.edPhone.Text:=Filters.Items[IndexFindPhone].Value;
  ...
  if fm.ShowModal=mrOk then begin
    ...
    //Filters.Items[IndexFindPhone].Value:=Trim(fm.edPhone.Text);
   // Filters.Items[IndexFindPhone].Enabled:=Trim(fm.edPhone.Text)<>'';
