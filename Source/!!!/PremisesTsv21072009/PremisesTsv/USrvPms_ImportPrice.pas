...
begin
 try
   try
    ...
    TCPB.Color:=clNavy;
    pb:=_CreateProgressBar(@TCPB);
    _SetSplashStatus('Импорт прайса');
    fmParent.MemTable.DisableControls;
    OldAfterScroll:=fmParent.MemTable.AfterScroll;
    fmParent.MemTable.AfterScroll:=nil;
   ...
   case Key of
    ...
    VK_F4: begin
      if not MemTable.isEmpty then begin
       but:=DeleteWarningEx('текущее предложение?');
...
begin
  ...
  vlEditor.TitleCaptions.Add('Привязка к полю');
...
begin
  ...
  IndexDateArrivals:=strCondition.Add('Дата постуления');
  IndexTypeRoom:=strCondition.Add('Тип комнат');
  IndexPlanning:=strCondition.Add('Планировка');
  IndexPhone:=strCondition.Add('Телефон');
  IndexSaleStatus:=strCondition.Add('Статус продажи');
  IndexDocument:=strCondition.Add('Документы');
  IndexTypeHouse:=strCondition.Add('Тип дома');
  IndexGeneralArea:=strCondition.Add('Общая площадь');
  IndexDwellingArea:=strCondition.Add('Жилая площадь');
  IndexKitchenArea:=strCondition.Add('Площадь кухни');
  IndexAllArea1:=strCondition.Add('Общая/Жилая/Кухня');
  IndexAllArea2:=strCondition.Add('Общая\Жилая\Кухня');
  IndexBalcony:=strCondition.Add('Балкон');
  IndexCondition:=strCondition.Add('Ремонт');
  IndexStove:=strCondition.Add('Плита');
  IndexSanitaryNode:=strCondition.Add('Санузел');
  IndexSelfForm:=strCondition.Add('Форма собственности');
  IndexTypePremises:=strCondition.Add('Тип недвижимости');
  IndexPrice:=strCondition.Add('Цена');
  IndexUnitPrice:=strCondition.Add('Единица измерения цены');
  IndexPriceUnitPrice:=strCondition.Add('ЦенаЕдиница измерения цены');
  IndexNote:=strCondition.Add('Примечание');
  IndexContact:=strCondition.Add('Контакт');
  IndexContactPlus:=strCondition.Add('Контакт (спец.)');
  IndexClientInfo:=strCondition.Add('Клиент');
  IndexAgent:=strCondition.Add('Агент');
  IndexStation:=strCondition.Add('Статус временной');
  IndexRegion:=strCondition.Add('Район');
  IndexStreet:=strCondition.Add('Улица');
  IndexHouseNumber:=strCondition.Add('Дом');
  IndexApartmentNumber:=strCondition.Add('Квартира');
  IndexHouseApartment1:=strCondition.Add('Дом-Квартира');
  IndexCountRoom:=strCondition.Add('Количество комнат');
  IndexFloor:=strCondition.Add('Этаж');
  IndexCountFloor:=strCondition.Add('Этажность');
  IndexFurniture:=strCondition.Add('Мебель');
  IndexDoor:=strCondition.Add('Дверь');
  IndexTerm:=strCondition.Add('Срок');
  IndexPayment:=strCondition.Add('Оплата за');
  IndexTermPayment1:=strCondition.Add('Срок\Оплата за');
  IndexTermPayment2:=strCondition.Add('Срок/Оплата за');
  IndexDateUpdate:=strCondition.Add('Дата обновления');
  IndexHeat:=strCondition.Add('Отопление');
  IndexWater:=strCondition.Add('Водоснабжение');
  IndexBuilder:=strCondition.Add('Застройщик');
  IndexDelivery:=strCondition.Add('Сдача');
  IndexNN:=strCondition.Add('Номер №№');
  IndexCityRegion:=strCondition.Add('Городской район');
  IndexPrice2:=strCondition.Add('Цена за м2');
  IndexGlassy:=strCondition.Add('Остекление');
  IndexDecoration:=strCondition.Add('Отделка');
end;
...
begin
  if not MemTable.IsEmpty then begin
    mr:=ShowQuestionEx('Очистить предварительный просморт?');
