unit URptPms_PriceLandClient1;

interface

uses classes, Forms;

procedure RtpLandRunClient1(Thread: TThread; Form, Options: TForm; sFileName, sTypeOperation: string;
...
begin
 ...
 try
   ...
   {while not qrObject.Eof do begin
     oldDirectionId:=-1;
     ShMain.Select;
     ShMain.Copy(After:=Sh);
     Sh:=Wb.Sheets.Item[incTP+1];
     Sh.Name:=qrObject.FieldByName('note').AsString;
     curY:=3;
       ...
       while not qrDirection.Eof do begin
         ...
         while not qrCityRegion.Eof do begin
           ...
           sqls:=SQLRbkPms_PremisesRpt+
                 ...
                 ' and p.pms_object_id='+qrObject.FieldByname('pms_object_id').AsString+
